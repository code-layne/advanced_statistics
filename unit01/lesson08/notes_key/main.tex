...
\small
\begin{multicols}{2}
\vocabans{Boxplot (box-and-whisker plot)}{A display of the five-number summary: a box from $Q_1$ to $Q_3$ with a median line, and whiskers to the min and max.}
\vocabans{Five-number summary}{$\min,\; Q_1,\; M,\; Q_3,\; \max$ --- the five values that define a boxplot.}
\vocabans{Box (IQR box)}{The rectangle from $Q_1$ to $Q_3$; spans the middle 50\% of the data.}
\columnbreak
\vocabans{Whisker}{The line from the box edge to the min or max (or last non-outlier in a modified boxplot).}
\vocabans{Modified boxplot}{Whiskers end at the last non-outlier; outliers are plotted individually as dots ($\bullet$).}
\vocabans{Fence (lower / upper)}{Outlier boundaries: lower $= Q_1 - 1.5\,\text{IQR}$; upper $= Q_3 + 1.5\,\text{IQR}$.}
\end{multicols}
\end{vocabbox}
