...
\begin{tcolorbox}[colback=sky, colframe=navy, boxrule=0.9pt, arc=2mm,
    ...
    Students construct only sentences --- they fill the pile-and-stragglers table at the end of
    notes section 1 and then write descriptions. Do not add a ``now draw a skewed
...
\begin{minipage}[t]{0.48\linewidth}
    ...
    \begin{itemize}
        ...
        \item \textbf{3.} A dotplot with one student far from the other fourteen.
              ...
              section~1 names spread and section~2 names the other two.
    \end{itemize}
\end{minipage}
\hfill
\begin{minipage}[t]{0.48\linewidth}
...
second half, \emph{Shape: Count the Peaks, Then Check the Mirror}, puts up the three histograms
and names unimodal, bimodal, uniform, and symmetric. Fill the pile-and-stragglers table row by
row with the room --- \emph{where does it pile, which way do the stragglers run} --- because
those two columns are exactly the evidence the skew name is read off in section~2. Do
\textbf{not} name the skew yet. Five minutes.

\textbf{Section 2 is the lesson.} Its first half names outlier, gap, and cluster off Line B's
two stranded customers, then hands over SOCS and the one-line limit on what a description may
claim. Four minutes. Its second half, \emph{Skewed Which Way? The Tail Decides}, is the crux.
Work the \texttt{work} block live: thirty of the forty homes sold under \$400{,}000, and the
axis still runs past \$700{,}000. Then say the name --- \emph{skewed right} --- and let the
protest happen. The sentence to land is \emph{the name follows the tail, not the pile}; say it,
write it, and point at Picture~I while you do. Mirror it on Picture~II. Then the two
counterweights: Picture~III is symmetric \emph{and} bimodal, and a reported ``typical height''
of 68 inches describes no adult in the club; and the 13-minute customer is checked, not
deleted. Close on all four parts for Picture~I, said aloud in context. Six minutes.
...
\begin{enumerate}[leftmargin=1.3em, nosep, itemsep=3pt]
  \item \textbf{Open on the almost-right answer.} Somebody in Guided Practice (b) wrote
...
\begin{itemize}[leftmargin=1.2em, nosep]
  ...
  \item \textbf{The outlier gets deleted} (notes~2, GP~(a), HW~5(D), AP~MC3(D)). Probe:
...
\textbf{item~2 is the column that tells you whether section~2 landed} --- a classmate has called
...
survived) --- and open Lesson~1.5 by reteaching the second half of section~2 if that third pile
...
the homework start.} Section~1 must move: five minutes on the checkout lines, five on the three
pictures. \textbf{Take the vote before the arithmetic.} If students compute the two middle waits
...
question they already have. \textbf{Fill the pile-and-stragglers table without naming the skew.}
Students will want the word; hold it until section~2, because the table is the evidence the
name is read off, and a name handed out first gets attached to the pile.

\textbf{Spend the time in section 2}, which carries the misconception. Its first half is quick
--- outlier, gap, cluster, SOCS, and the limit on what a description may claim --- so the
second half gets six minutes. Work the \texttt{work} block live --- 30 of the 40 homes under
\$400{,}000, and the axis running past \$700{,}000 --- and \emph{then} say ``skewed right.'' Let
the protest happen; it is the whole point. Do not resolve it by repeating the definition. Point
at the tail.

\textbf{Do not skip the counterweights} at the end of section~2: Picture~III, symmetric and
bimodal, center 68 inches, nobody there; and the 13-minute customer, checked rather than
deleted. Without the first, students leave believing that ``symmetric'' finishes a description,
which is the version of this error that costs points on the free response.
...
\textbf{Debrief (8 min), spoken.} Open on the almost-right answer from Guided Practice (b), then
