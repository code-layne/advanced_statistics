...
\begin{tcolorbox}[colback=sky, colframe=navy, boxrule=0.9pt, arc=2mm,
    ...
    problem~5), name what the pictures show, and write descriptions. Do not add a ``now draw a skewed
...
Warm-Up & 5 & Two pictures of 40 fish lengths, a middle pumpkin, and one student who read 15
  ...
  board all period \\
Guided Notes \& Practice & 34 & Fill the vocabulary box and the notes table: problems 1--15
  row by row, then \emph{The Sleep Survey} (16--19) together, pen in their hand & \textbf{I do}
  each row's definition, its display, and first problem, \textbf{we do} the rest of its grid (24) $\to$
...
the vote on the board. Read the definition, then mark up the two dotplots: count in from both
ends and write \emph{4 min} in both label boxes --- now the room has a problem worth five
minutes. Work problem~1 yourself; problems 2--4 are theirs, and center and spread get named
against that vote. Hold the trap --- ``the typical wait is 4 minutes in both lines'' --- for
problem~15 in The Tail row.

\textbf{Peaks \& Mirror (5 min).} Read the definition, then mark up the three histograms:
name the peaks in the three label boxes (unimodal, unimodal, bimodal). Problem~5 is the
pile-and-stragglers table --- fill its first row yourself, then the room fills II and III,
\emph{where does it pile, which way do the stragglers run}, because those two columns are
exactly the evidence the skew name is read off in The Tail. Do \textbf{not} name the skew
yet; problems 6--7 stay on the mirror and on uniform.

\textbf{Unusual Features (5 min).} Read the definitions, then mark up Line B: the two
stranded customers are \emph{outliers}, the empty stretch is a \emph{gap} --- both go in the
label boxes --- then SOCS. Work problem~8 yourself; problem~9 is where ``in context'' stops
being a slogan --- make the rewrite name the line, the wait, and the minutes; problem~10 is the
13-minute customer, checked rather than deleted; and problem~11 is the one-line limit on what a
description may claim.

\textbf{The Tail (9 min) --- the crux.} Read the count off the enlarged Picture~I --- 30 of
the 40 under \$400{,}000, and the axis still runs past \$700{,}000 --- point at the pile and
the tail, and \emph{then} write the name, \emph{right}, in the label box, and let the protest
happen. The sentence to land is \emph{the name follows the tail, not the pile}; say it, point
at the callout, and point at Picture~I while you do. Say all four for Picture~I in context and
have them write it in their own words. Then the grid: \textbf{problem~12 is the crux} ---
Picture~II, pile on the right, skewed \emph{left}; take a hand count before anyone speaks.
Problem~13 is the same error in a classmate's mouth. \textbf{Problems 14--15 are the
counterweights}: Picture~III is symmetric \emph{and} bimodal, ``about 68 inches'' describes
no adult in the club; and \textbf{problem~15 is the trap} --- ``the typical wait is 4 minutes
in both lines'' is true and useless. Make them say both halves. Do not skip them. If time
allows, cold-call a skewed-right variable of their own.

\columnbreak
\textbf{We do (about 10 min): The Sleep Survey.} A dotplot in a fresh context, problems 16--19,
...
\textbf{Problem~16 is the crux transferred}, and \textbf{problem~19 is the whole lesson in one
...
\begin{itemize}[leftmargin=1.1em, nosep]
  \item \textbf{Problem~16, the crux transferred:} ``Point at the tail. Now say the name.'' A
        student who answers \emph{skewed right} is reading the pile.
  \item \textbf{Problem~17:} ``Read me the gap. Where does it start and where does it end?''
  \item \textbf{Problem~19:} ``You wrote \emph{about 8}. Eight what? Of what? Say the units.''
\end{itemize}
While circulating, pick the papers to put up in the debrief --- one problem~19 that named all
...
\begin{enumerate}[leftmargin=1.3em, nosep, itemsep=3pt]
  \item \textbf{Open on the almost-right answer.} Somebody in problem~16 wrote
...
\begin{itemize}[leftmargin=1.2em, nosep]
  \item \textbf{Skew named for the pile} (notes~12--13, GP~16, HW~1--2, AP~MC2 --- the target
        misconception). Probe: \emph{``Point at the tail. Now say the name.''}
  \item \textbf{The outlier gets deleted} (notes~10, HW~5(D), AP~MC3(D)). Probe:
        \emph{``If you throw that customer out, what do you no longer know about this line?''}
  \item \textbf{Center reported alone} (notes~15, GP~19, AP~FR(c)): the Line A / Line B trap
        surviving to the end of the period. Probe: \emph{``Both lines average four minutes.
        Which one do I join?''}
  \item \textbf{Description with no context} (notes~9, GP~19, HW~4, AP~FR(b)): ``unimodal,
        skewed right, center 6, spread 19'' with no variable and no units. Probe: \emph{``Six
        what?''}
  \item \textbf{Symmetric treated as the whole answer} (notes~14, HW~3): Picture~III is
        symmetric \emph{and} bimodal. Probe: \emph{``How many people are at the center?''}
  \item \textbf{Over-claiming beyond the data} (notes~11, AP~MC3, HW~5(C)): from 40 commutes to
...
a short definition, one large display to mark up, and a grid of short problems. \textbf{In
every row you read the definition, mark up the picture, and work the first problem; the class
works the rest with the pen in their hand.} The first
...
in the two label boxes, center and variability arrive as the answer to a question they already
have. \textbf{Fill the pile-and-stragglers table (problem~5) without naming the skew.} Students will
...
\textbf{Spend the time in The Tail}, which carries the misconception. Read the count off the
enlarged Picture~I --- 30 of the 40 under \$400{,}000, and the axis running past \$700{,}000 ---
and \emph{then} write ``right'' in the label box. Let the protest happen; it is the whole
point. Do not resolve it by repeating the definition. Point at the tail. Problem~12 is the
crux --- Picture~II, pile on the right, skewed \emph{left} --- and problem~13 is the same error
in a classmate's mouth.

\textbf{Do not skip problems 14--15}, the counterweights: Picture~III, symmetric and bimodal,
center 68 inches, nobody there; and the trap, the two checkout lines with the same typical wait
--- true and useless without the spread. Without the first, students leave believing that
``symmetric'' finishes a description, which is the version of this error that costs points on
the free response. (The 13-minute customer, checked rather than deleted, is problem~10 in
Unusual Features.)

\textbf{The Sleep Survey is where the pen changes hands.} It is a dotplot in a fresh context,
and problem~16 is the crux transferred --- the pile is on the right and the tail runs left, so
a student reading the pile will answer \emph{skewed right}. Problem~19 is the whole lesson in
one answer. If you only get through 16, 17, and 19, that is the right trade.
...
\textbf{Debrief (8 min), spoken.} Open on the almost-right answer from problem~16, then
