%! Describing the Distribution of a Quantitative Variable — Unit 1, Lesson 1.4 — Homework (Answer Key)
...
\begin{enumerate}[leftmargin=1.9em, itemsep=5pt]
  \item \textbf{Match each display to its description.} Write the letter of the display on the
        ...
        \begin{enumerate}[label=\textbf{(\roman*)}, leftmargin=2.4em, itemsep=3pt]
          \item Roughly symmetric, with a single peak in the middle. \ans{A}
          ...
          \item Skewed to the left: most values are large, and a few are much smaller.
                \ans{C}
          \item Bimodal: two clear peaks with a gap between them. \ans{D}
        \end{enumerate}
  ...
  \item A student looked at \textbf{Display B} and wrote: \emph{``This distribution is skewed to
        ...
        name.
        \answerspace{2.4cm}{Display B is skewed to the \emph{right}. A skew is named for the
        side the \emph{tail} runs out on, not the side the pile sits on: the values bunch at the
        low end and a few stragglers stretch far to the high end, so the long tail is the right
        one. The student named the picture by where the data \emph{is}.}
  \item \textbf{Display D} shows the ages, in years, of the 22 people at one community swim
        ...
        \answerspace{2.2cm}{Display D is bimodal: two clear peaks separated by a gap, so the
        ages form two clusters with no one observed in between. A plausible reason is that the
        session drew young children together with the adults who brought them, and almost no
        teenagers or young adults came.}
  \item The dotplot below shows the grams of sugar per serving in each of 24 breakfast cereals.
        ...
        \answerspace{2.8cm}{The sugar content of these 24 cereals is unimodal and skewed to the
        right. The center is about 6 grams per serving, and the values run from 2 to 21 grams,
        with most cereals between 3 and 9 grams. Two cereals sit far above the rest at 20 and
        21 grams, behind an empty stretch from 13 to 20 grams. This describes these 24 cereals
        only --- not every cereal on the shelf.}
  \item \textbf{AP-style multiple choice.} A histogram of the salaries of all 250 employees at
        ...
        \begin{enumerate}[label=\textbf{(\Alph*)}, leftmargin=2.2em, itemsep=2pt]
          ...
          \item \ans{Most of these 250 employees earn near the lower end, while a few earn much more.}
        ...
        \end{enumerate}
        \answerspace{2.0cm}{(B). A right tail means the pile of salaries is at the low end with
        a few much larger ones stretching out --- and the display describes only these 250
        employees, not every company in the industry.}
\end{enumerate}
\end{notesbox}
...
actual numbers on the middle of a distribution --- the \textbf{mean} and the \textbf{median} ---
and on how spread out it is --- the \textbf{IQR} and the \textbf{standard deviation} --- and we
find out which of those numbers one strange value can drag around, and which one barely notices
it.
\end{spiralbox}
