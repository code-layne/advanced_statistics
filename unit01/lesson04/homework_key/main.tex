...
\begin{enumerate}[leftmargin=1.9em, itemsep=6pt]
  ...
  \item A student looked at \textbf{Display B} and wrote: \emph{``This distribution is skewed to
        ...
        name.
        \answerspace{3.1cm}{Display B is skewed to the \emph{right}. The name of a skew comes
        from the side the \emph{tail} runs out on, not the side the pile sits on. In Display B
        the values bunch at the low end and a few stragglers stretch far out to the high end, so
        the right tail is the long one --- skewed right. The student named the picture by where
        the data is instead of by where the data runs out.}
  \item \textbf{Display D} shows the ages, in years, of the 22 people at one community swim
        ...
        \answerspace{2.8cm}{Display D is bimodal: two clear peaks separated by a gap, so the
  ...
  \item The dotplot below shows the grams of sugar per serving in each of 24 breakfast cereals.
        ...
        \begin{tikzpicture}[scale=0.95]
          ...
          \node[font=\scriptsize] at (2.8,-0.6) {Sugar per serving (grams)};
        \end{tikzpicture}
        \end{center}
        \vspace{2pt}
        ...
        \answerspace{3.4cm}{The sugar content of these 24 breakfast cereals is unimodal and
        skewed to the right. The center is about 6 grams of sugar per serving, and the values
        run from 2 grams to 21 grams, with most cereals between 3 and 9 grams. Two cereals sit
        far above the rest at 20 and 21 grams, separated from the others by an empty stretch
        from 13 to 20 grams. This describes only these 24 cereals --- it is not a claim about
        every cereal on the shelf.}
  \item \textbf{AP-style multiple choice.} A histogram of the salaries of all 250 employees at
        ...
        \end{enumerate}
        \answerspace{2.4cm}{(B). A right tail means the pile of salaries is at the low end with
