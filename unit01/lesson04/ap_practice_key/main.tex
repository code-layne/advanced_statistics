...
\begin{enumerate}[leftmargin=1.9em, itemsep=0.13in]
  ...
  \item Which statement best describes the shape of this distribution?
        \begin{enumerate}[label=\textbf{(\Alph*)}, leftmargin=2.2em, itemsep=2pt]
          ...
          \item Skewed to the left, because most of the commutes lie on the left side of the
                graph.
          \item \ans{Skewed to the right, because the tail of longer commutes stretches out to the
                right.}
  ...
  \item Which conclusion is \textbf{supported} by this display?
        \begin{enumerate}[label=\textbf{(\Alph*)}, leftmargin=2.2em, itemsep=2pt]
          ...
          \item \ans{Among these 40 employees, most commute less than 30 minutes, and one commutes
  ...
  \item Before concluding that this distribution is skewed to the right, a careful reader should
        ...
        \begin{enumerate}[label=\textbf{(\Alph*)}, leftmargin=2.2em, itemsep=2pt]
          ...
          \item \ans{the interval width used, because a different width can change the appearance of
                the histogram.}
...
\begin{enumerate}[leftmargin=1.9em, itemsep=0.16in, resume]
  ...
  \item Use the commute record above.
        ...
        \ansline{Quantitative --- a commute time is a measured amount, not a category. It is}\\[1pt]
        ...
        in context.\\[4pt]
        \ansline{Skewed to the right: most of these 40 employees commute under 30 minutes, and}\\[1pt]
        \ansline{the tail of longer commutes stretches to the right. One employee is an outlier,}\\[1pt]
        \ansline{commuting 80--90 minutes, cut off from the rest by an empty stretch of 20 minutes.}
        ...
        \ansline{It leaves out the shape, the spread, and the unusual value. These commutes run}\\[1pt]
        \ansline{from under 10 minutes to over 80, and one employee travels more than 80 minutes}\\[1pt]
        \ansline{each way --- a person ``about 25 minutes'' hides entirely. These data describe only}\\[1pt]
        \ansline{these 40 employees; about other companies they can only suggest what to check.}
        ...
        \ansline{That employee lands in a 60--90 interval together with the two 50--60 commutes,}\\[1pt]
        \ansline{so the long commute stops standing apart. Changing the width changes what the}\\[1pt]
        \ansline{picture shows without changing a single value --- check the width first.}
...
\ansline{Answers vary. A single average always leaves out shape, spread, and unusual}\\[1pt]
\ansline{features; the usual result is that a long right tail is read as a typical value.}
\end{extensionbox}
