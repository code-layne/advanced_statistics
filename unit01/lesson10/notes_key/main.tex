...
\newcommand{\vocabans}[2]{%
  ...
  \ansline{#2}\\[1pt]\writeline\par\vspace{3pt}}
...
\small
\begin{multicols}{2}
\vocabans{Normal distribution}{Symmetric, bell-shaped distribution described by $\mu$ and $\sigma$; notation $N(\mu,\sigma)$.}
\vocabans{68--95--99.7 Rule (Empirical Rule)}{$\approx$68\%, 95\%, 99.7\% of data fall within 1, 2, 3 SDs of the mean.}
\vocabans{$z$-score (standardized value)}{$z = (x-\mu)/\sigma$; the number of SDs above or below the mean.}
\columnbreak
\vocabans{Standard Normal distribution $N(0,1)$}{Normal distribution with $\mu=0$ and $\sigma=1$; all $z$-scores follow this.}
\vocabans{Normal table (Table A)}{Gives the cumulative area to the LEFT of a given $z$-score.}
\vocabans{Normal probability plot (NPP)}{If points fall near a diagonal line, the distribution is approximately Normal.}
\end{multicols}
\end{vocabbox}
...
\begin{itemize}[itemsep=3pt]
  \item Shape: \ans{unimodal, symmetric} (bell-shaped)
  \item Symmetric about the \ans{mean}
  \item Mean $=$ Median $=$ Mode
  \item Described by \ans{2} parameters:
        $\mu$ (\ans{mean}) and $\sigma$ (\ans{std.\ deviation})
  \item Notation: \ans{$N(\mu,\sigma)$}
  \item Tails extend to $\pm\infty$ but curve
        approaches \ans{zero} quickly
\end{itemize}

\vspace{6pt}
\textbf{\textcolor{navy}{Changing $\mu$ shifts the curve:}}\\
Moving $\mu$ left or right \ans{does not change} the shape.
...
\vspace{4pt}
\small The curve is \ans{tallest (peaked)} at $\mu = 70$
...
$P(\text{height} > 76) = \dfrac{100\% - 95\%}{2} = \ans{2.5\%}$
...
\[
  z = \dfrac{x - \mu}{\sigma}
\]
Interpretation: $z$ tells us how many \ans{standard deviations}
above or below the \ans{mean} the value $x$ falls.

\vspace{5pt}
\textbf{\textcolor{navy}{Standardizing:}} Converting $x$ to $z$ transforms
any $N(\mu, \sigma)$ into the \ans{Standard}
Normal distribution: $N(\ans{0},\ans{1})$.

\vspace{5pt}
\textbf{\textcolor{navy}{Example (heights $N(70,3)$):}}\\
$x = 76$: $z = \dfrac{76-70}{3} = \ans{$+2$}$\\[4pt]
$x = 64$: $z = \dfrac{64-70}{3} = \ans{$-2$}$\\[4pt]
$x = 70$: $z = \dfrac{70-70}{3} = \ans{0}$

\columnbreak

\textbf{\textcolor{navy}{Using Table A (Normal table):}}\\
Table A gives $P(Z < z)$ = area to the \ans{left}.
...
\begin{enumerate}[itemsep=3pt, label=\alph*.]
  \item \textbf{Histogram} (or dotplot): look for a
        \ans{symmetric}, \ans{unimodal} shape with
        no strong \ans{skewness}.
  \item \textbf{Normal probability plot (NPP)}:
        if the points fall \ans{close to} a
        diagonal line, the distribution is approximately
        Normal. \ans{Curves} from a line suggest skewness.
\end{enumerate}

\vspace{5pt}
\textbf{\textcolor{navy}{When can we use the Normal model?}}\\
Use it when: the histogram is \ans{symmetric} and
\ans{bell-shaped}, or the NPP is \ans{linear}. Never
use it blindly without \ans{checking} first.
...
\vspace{4pt}
\small
Left plot: points near the line $\Rightarrow$ \ans{approximately} Normal.\\
Right plot: points curve away $\Rightarrow$ \ans{skewed}.
...
\hfill $z = \ans{$+2.0$}$\\[4pt]
(c) Is 9.7 lb unusually high? \ans{Yes} \quad Justify: \ans{$z=+2$; only $\approx2.5\%$ weigh more}
\end{tcolorbox}
