...
\begin{frame}[t, plain]
  ...
  \begin{minipage}{0.9\textwidth}
    ...
    \begin{itemize}
      \setlength{\itemsep}{0.5em}
      \item \textcolor{white}{The {\color{goldacc}\bfseries Normal distribution} $N(\mu, \sigma)$ is symmetric, unimodal,
            bell-shaped; fully described by mean and SD.}
      \item \textcolor{white}{The {\color{goldacc}\bfseries 68--95--99.7 Rule} gives approximate proportions within
            1, 2, 3 standard deviations of the mean.}
      \item \textcolor{white}{A {\color{goldacc}\bfseries $z$-score} standardizes any value: $z = (x-\mu)/\sigma$.
            It counts standard deviations from the mean.}
      \item \textcolor{white}{{\color{goldacc}\bfseries Table A} gives $P(Z < z)$ (area to the left).
            Subtract from 1 for ``greater than''; subtract areas for ``between''.}
      \item \textcolor{white}{Always {\color{goldacc}\bfseries assess Normality} with a histogram or Normal probability plot
