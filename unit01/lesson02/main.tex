...
% row by row in the Main Ideas / Notes table -> we do (the Guided Practice row). THERE IS NO
% INDEPENDENT PRACTICE SET — the homework is
...
\begin{tcolorbox}[colback=sky, colframe=navy, boxrule=0.9pt, arc=2mm,
    ...
    \texttt{COURSE\_RESTRUCTURE.md}. That is a lot for 55 minutes. The \emph{Counts \& Shares}
    and \emph{About Half} rows carry both tables, the \emph{Bar Graphs} row keeps the mechanics
    to one pre-drawn chart, and the \emph{Wins First} row --- the idea both topics share,
    \textbf{counts versus rates} --- gets the time. If the notes run long, trim the Bar Graphs
    grid, never the crux (problem~12).
    \\[2pt]
...
Guided Notes \& Practice & 34 & Fill the vocabulary box and the notes table: problems 1--15 row by row, then \emph{Two Shifts} (16--19) together, pen in their hand & \textbf{I do} each row's definition, its display, and its first problem, \textbf{we do} the rest of its grid (24) $\to$ \textbf{we do} Guided Practice (10) \\
...
\begin{minipage}[t]{0.48\linewidth}
    ...
    \begin{itemize}
        ...
        \item \textbf{2.} $80/400$ and $150/1000$ as percents. \textbf{Seeds:} the exact
              arithmetic the \emph{Wins First} row needs (problem~10), done before the crux so
...
\begin{minipage}[t]{0.48\linewidth}
    \textbf{Running it}
    \begin{itemize}
        \item Item~3 is the crux of the \emph{Wins First} row in prior-knowledge terms.
              \textbf{Debrief it aloud} and press the second half: \emph{why} the raw win totals
              did not decide it. Write their phrasing on the board verbatim and leave it up.
        \item You will point at that sentence twice more --- when problems 11--12 land, and
              again in the closing debrief.
...
The notes are one \emph{Main Ideas / Notes} table --- five rows, 19 short problems. \textbf{In
every row you read the definition, mark up the display, and work the first problem of the grid;
the class works the rest, pen in their hand.} The vocabulary box is filled \emph{as you go}, never in advance.
\begin{multicols}{2}
\textbf{Counts \& Shares (6 min).} Both tables off one column. Read the definition, name the
two columns (\emph{count}, \emph{share}), land the two totals as \textbf{two different
promises} --- 400, and 1 --- then problem~1: do the Bus share yourself and hand them the other
four. Problem~2 makes them say the totals back;
\textbf{problem~3 is \emph{Proportion, Percent, Rate} in one cell} --- $80/400 = 0.20 = 20\% = $
1 in 5 --- and the AP habit of attaching context rides on it. Keep it under a minute.

\textbf{About Half (3 min).} Plurality against majority. Point at the \emph{half of North} line on
the stacked bar: Bus stops short. \textbf{Problem~4 is the
plurality trap}: work the share yourself (45\%), then take a hand count on ``is that a majority?''
before anyone speaks, and make them say what the other 55\% are doing. Problem~5 is the context
habit: not ``20\%'' but \emph{20\% of North students}.

\textbf{Bar Graphs (6 min).} The only graph block: bar $=$ category, height $=$ count \emph{or}
relative frequency, gaps because categories are not a number line. Fill the two boxes under the
chart (\emph{category}, \emph{count}), work problem~6 yourself; 7--9 are theirs, read off the
printed chart. Problem~8 is the gap sentence
they will need in Lesson~1.3, and \textbf{problem~9 is the reading habit} --- \emph{what is the
vertical axis counting?}

\textbf{Wins First (9 min) --- the crux.} Point at the district's side-by-side chart before
computing anything: could South have lost \emph{any} category on raw counts? South is taller
almost everywhere, and that was guaranteed. Read the rule, have them write the \emph{In your own
words} line, then problem~10 yourself, both rates on the board. Problems 11--12
land the lesson: put both true statements up together, and \textbf{problem~12 is the crux} ---
``which school walks more'' has no honest answer until you say which question. Problem~13 is the
fix; \textbf{problem~14 is the counterweight} (``how many'' wants a count --- do not skip it);
\textbf{problem~15 is the wrong-total trap} (150 out of 1{,}400 is a share of nothing anyone
asked about).

\columnbreak
\textbf{We do (about 10 min): Two Shifts.} A fresh context, problems 16--19, worked entirely
together. Students hold the pen; you hold the questions: \emph{``Which shift sold more? Which
sells them more often? Are those the same question?''} \quad \emph{``Busier --- is that about
how many, or about what share?''} \textbf{Problem~19 is the counterweight again} and is the one
to protect when time is short.
...
on \emph{which shift sells large drinks at the higher rate} (problem~17) before anyone speaks,
and make the room defend both numbers.

Circulate and demand the reason, not the letter:
\begin{itemize}[leftmargin=1.1em, nosep]
  \item \textbf{Problem~17, the crux transferred:} ``Go back to the warm-up. Team B won more
        games. Was Team B better?''
  \item \textbf{Problem~18:} ``Which kind of number did you write the sentence with --- and why
        that one?''
  \item \textbf{Problem~19:} ``Busier --- how many, or what share? Point at the column.''
\end{itemize}
While circulating, pick the papers to put up in the debrief --- one that answered problem~17 with
...
\begin{enumerate}[leftmargin=1.3em, nosep, itemsep=3pt]
  ...
  \item Take the plurality trap next, off problem~4 --- 45\% is the largest share, and 55\% of
        North does something else. Land it as a reading habit, not a vocabulary item.
  \item Take the wrong-total trap last, off problem~15: \emph{percent of what?} Point at the
        two-schools table in the \emph{Wins First} row and ask what dividing by 1,400 computed.
\end{enumerate}
\columnbreak
...
\begin{itemize}[leftmargin=1.2em, nosep]
  \item \textbf{Counts compared across different-sized groups} (problems 11--12 and~17, HW~2
        and~5, AP~MC2 --- the target misconception). Probe: \emph{``Team B won more games. Was
        Team B better?''}
  \item \textbf{Plurality read as majority} (problem~4, HW~1). Probe: \emph{``45\% chose the
        bus. What did the other 55\% do?''}
  \item \textbf{Percent taken of the wrong total} (problem~15) --- dividing by 1,400 (both
        schools) or by the category rather than the group. Probe: \emph{``Percent \emph{of
        what}? Point at the number you divided by.''}
  \item \textbf{Relative frequencies not summing to 1} (problems 1 and~2, HW~4, AP~MC3):
        students treat the column as four unrelated numbers. Probe: \emph{``Add your column.
        What should it come to, and why must it?''}
  \item \textbf{Rate applied where a count is wanted} (problems 14 and~19): over-correction
        after the crux lands. Probe: \emph{``Busier --- how many, or what share?''}
...
whether the crux (problem~12) landed. Sort what you see into three piles as you walk
...
has, and its numbers are deliberately \emph{not} the ones the \emph{Wins First} row uses, so the
transfer is real. Give three minutes and make sure the second half gets answered out loud: \emph{why} Team
...
a printed definition, a display, and a grid of short problems. \textbf{In every
row you read the definition, mark up the display, and work the first problem; the class works
the rest with the pen in their hand.}
\textbf{This lesson covers two CED topics, so the instruction rows are the tightest of the
unit.} \emph{Counts \& Shares} and \emph{About Half} are the tables and must move: nine minutes
between them, with problem~3 (proportion, percent, rate) under a minute and problem~4 (the
plurality trap) taken on a hand count. \emph{Bar Graphs} is the only stretch of pure display
instruction --- keep it to six minutes and keep pointing at the printed chart; problems 6--9 are
read off it, which is what stops the row becoming a lecture.

\textbf{Spend the time in \emph{Wins First}}, which carries the misconception. Before you compute
anything, point at the district's side-by-side chart and ask whether South could have lost any
category. Then work problem~10 live and put both true statements on the board at once
(problem~11) --- \emph{South has more walkers} and \emph{North has the higher rate}. Do not
resolve problem~12 too fast; the sentence to land is that they answer two different questions.

\textbf{Do not skip problem~14, the counterweight} (``how many students in this district
walk?''). Without it students leave with ``always use rates,'' which is wrong, and they will
over-apply it on the AP exam. Problem~15 is the wrong-total trap and takes thirty seconds ---
\emph{percent of what?}

\textbf{Guided Practice is where the pen changes hands.} \emph{Two Shifts} reverses the numbers:
morning sells more large drinks (84 vs.\ 32, problem~16) while evening sells them at the higher
rate (40\% vs.\ 35\%, problem~17). Problem~19 is the counterweight in its cleanest form and is the
one to protect when you are running behind. If you only get through 16, 17, and 19, that is the
right trade.
...
\emph{Counts \& Shares} fills (problems 1--2) did not land. \textbf{When you score the page, sort item~5 into three piles} ---
