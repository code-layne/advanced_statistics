...
\begin{tcolorbox}[colback=sky, colframe=navy, boxrule=0.9pt, arc=2mm,
    ...
    grid, never the crux (problem~16).
    \\[2pt]
...
Guided Notes \& Practice & 34 & Fill the vocabulary box and the notes table: problems 1--19 row by row, then \emph{Two Shifts} (20--25) together, pen in their hand & \textbf{I do} each row's definition lines and first problem, \textbf{we do} the rest of its grid (24) $\to$ \textbf{we do} Guided Practice (10) \\
...
\begin{minipage}[t]{0.48\linewidth}
    ...
    \begin{itemize}
        ...
        \item \textbf{2.} $80/400$ and $150/1000$ as percents. \textbf{Seeds:} the exact
              arithmetic the \emph{Wins First} row needs (problem~14), done before the crux so
...
\begin{minipage}[t]{0.48\linewidth}
    ...
    \begin{itemize}
        ...
        \item You will point at that sentence twice more --- when problems 15--16 land, and
...
\begin{multicols}{2}
\textbf{Counts \& Shares (6 min).} Both tables off one column. Fill the definition lines and land
the two totals as \textbf{two different promises} --- 400 students, and 1 --- then problem~1: do
the Bus share yourself and hand them the other four. Problems 2--3 make them say the two totals
back; \textbf{problem~4 is \emph{Proportion, Percent, Rate} in one cell} --- $80/400 = 0.20 =
20\% = $ 1 in 5 --- and the AP habit of attaching context rides on it. Keep it under a minute.

\textbf{About Half (3 min).} Plurality against majority, in three problems. Work problem~5 yourself
(45\%); \textbf{problem~6 is the plurality trap} --- take a hand count on ``is that a majority?''
before anyone speaks, and make them say what the other 55\% are doing. Problem~7 is the context
habit: not ``20\%'' but \emph{20\% of North students}.

\textbf{Bar Graphs (6 min).} The only graph block: bar $=$ category, height $=$ count \emph{or}
relative frequency, gaps because categories are not a number line. Work problem~8 yourself; 9--13
are theirs, read straight off the printed chart rather than watched. Problem~11 is the gap
sentence they will need in Lesson~1.3, and \textbf{problem~13 is the reading habit} --- \emph{what
is the vertical axis counting?}
...
almost everywhere, and that was guaranteed. Fill \emph{2.5 times}, the comparison rule, and
\emph{how many}; then problem~14 yourself, both rates on the board. Problems 15--16 land the
lesson: put both true statements up together, and \textbf{problem~16 is the crux} --- ``which
school walks more'' has no honest answer until you say which question. Problem~17 is the fix;
\textbf{problem~18 is the counterweight} (``how many'' wants a count --- do not skip it);
\textbf{problem~19 is the wrong-total trap} (150 out of 1{,}400 is a share of nothing anyone
asked about).

\columnbreak
\textbf{We do (about 10 min): Two Shifts.} A fresh context, problems 20--25, worked entirely
...
how many, or about what share?''} \textbf{Problem~25 is the counterweight again} and is the one
to protect when time is short.

\textbf{The pen must actually change hands.} Guided Practice is the only place today where you
can watch a student decide, so do not narrate it. Put problem~20 on them cold, take a hand count
on \emph{which shift sells large drinks at the higher rate} (problem~23) before anyone speaks,
and make the room defend both numbers.

Circulate and demand the reason, not the letter:
\begin{itemize}[leftmargin=1.1em, nosep]
  \item \textbf{Problem~23, the crux transferred:} ``Go back to the warm-up. Team B won more
        games. Was Team B better?''
  \item \textbf{Problem~24:} ``Which kind of number did you write the sentence with --- and why
        that one?''
  \item \textbf{Problem~25:} ``Busier --- how many, or what share? Point at the column.''
\end{itemize}
While circulating, pick the papers to put up in the debrief --- one that answered problem~23 with
...
\begin{enumerate}[leftmargin=1.3em, nosep, itemsep=3pt]
  ...
  \item Take the plurality trap next, off problem~6 --- 45\% is the largest share, and 55\% of
        North does something else. Land it as a reading habit, not a vocabulary item.
  \item Take the wrong-total trap last, off problem~19: \emph{percent of what?} Point at the
...
\begin{itemize}[leftmargin=1.2em, nosep]
  \item \textbf{Counts compared across different-sized groups} (problems 15--16 and~23, HW~2
        and~5, AP~MC2 --- the target misconception). Probe: \emph{``Team B won more games. Was
        Team B better?''}
  \item \textbf{Plurality read as majority} (problem~6, HW~1). Probe: \emph{``45\% chose the
        bus. What did the other 55\% do?''}
  \item \textbf{Percent taken of the wrong total} (problem~19) --- dividing by 1,400 (both
        schools) or by the category rather than the group. Probe: \emph{``Percent \emph{of
        what}? Point at the number you divided by.''}
  \item \textbf{Relative frequencies not summing to 1} (problems 1 and~3, HW~4, AP~MC3):
        students treat the column as four unrelated numbers. Probe: \emph{``Add your column.
        What should it come to, and why must it?''}
  \item \textbf{Rate applied where a count is wanted} (problems 18 and~25): over-correction
...
whether the crux (problem~16) landed. Sort what you see into three piles as you walk
...
and work the first problem; the class works the rest with the pen in their hand.}
...
between them, with problem~4 (proportion, percent, rate) under a minute and problem~6 (the
plurality trap) taken on a hand count. \emph{Bar Graphs} is the only stretch of pure display
instruction --- keep it to six minutes and keep pointing at the printed chart; problems 8--13 are
...
category. Then work problem~14 live and put both true statements on the board at once
(problem~15) --- \emph{South has more walkers} and \emph{North has the higher rate}. Do not
resolve problem~16 too fast; the sentence to land is that they answer two different questions.

\textbf{Do not skip problem~18, the counterweight} (``how many students in this district
walk?''). Without it students leave with ``always use rates,'' which is wrong, and they will
over-apply it on the AP exam. Problem~19 is the wrong-total trap and takes thirty seconds ---
\emph{percent of what?}

\textbf{Guided Practice is where the pen changes hands.} \emph{Two Shifts} reverses the numbers:
morning sells more large drinks (84 vs.\ 32, problem~20) while evening sells them at the higher
rate (40\% vs.\ 35\%, problem~23). Problem~25 is the counterweight in its cleanest form and is the
one to protect when you are running behind. If you only get through 20, 23, and 25, that is the
right trade.
...
\emph{Counts \& Shares} fills (problems 1--3) did not land. \textbf{When you score the page, sort item~5 into three piles} ---
