...
table} lists how many cases fall in each category; the counts add to the \ans{group size}.
A \textbf{relative frequency table} lists the share of the group in each category; the shares
always add to \ans{1 (or 100\%)}, because every case is counted in exactly \ans{one} category.
...
\pcell{2}{What do the four counts add to, and why must they?}{1.2cm}{400: each student is in one category} &
\pcell{3}{What do the four shares add to?}{1.2cm}{1 (100\%)} &
\pcell{4}{Eighty of 400 walk. Write that share as a fraction, a decimal, a percent, and a
rate.}{1.2cm}{$80/400 = 0.20 = 20\%$ = 1 in 5} \\ \hline
\end{probgrid}
\\ \hline
% ── ROW 2: plurality vs majority, and the context habit ──────────────────────
\mainidea[``Most'' is a promise]{About half} &
The largest single category is a \ans{plurality}. A \textbf{majority} needs more than
\ans{half} of the group --- not the same thing. On the AP exam a share always comes with
its \ans{context}: not ``20\%,'' but \emph{20\% \textbf{of North students} walk to school}.
\notesprompt{Check the claim.}
\begin{probgrid}{|Y|Y|Y|}
\pcell{5}{A flyer says ``Most North students take the bus.'' What share takes the bus?}{1.2cm}{45\% --- a plurality} &
\pcell{6}{Is that a majority? Say why or why not.}{1.2cm}{No: the other 55\% do something else} &
\pcell{7}{Rewrite ``20\%'' as a sentence an AP reader would accept.}{1.2cm}{20\% of North students walk to school} \\ \hline
\end{probgrid}
\\ \hline
% ── ROW 3: bar graphs ────────────────────────────────────────────────────────
\mainidea{Bar graphs} &
The same North High column, drawn. Each bar stands for one \ans{category}, and its height is
the \ans{count} in that category --- or the \ans{relative frequency}, if the vertical axis is relabelled.
The bars have \ans{gaps} between them: categories are separate groups, not points on a
...
\begin{probgrid}{|Y|Y|Y|}
\pcell{8}{How tall is the Walk bar, and what is it counting?}{1.0cm}{80 students walk} &
\pcell{9}{What share of North is that?}{1.0cm}{20\%} &
\pcell{10}{Add the four bar heights. What do you get?}{1.0cm}{400, the group size} \\ \hline
\pcell{11}{Why are there gaps between the bars?}{1.3cm}{Categories are not a number line} &
\pcell{12}{Relabel the axis 0.45, 0.30, 0.20, 0.05. Which bars change height?}{1.3cm}{None --- only the axis labels change} &
\pcell{13}{Before reading any bar graph, what is the first question to ask?}{1.3cm}{What is the vertical axis counting?} \\ \hline
\end{probgrid}
\\ \hline
...
counted anything, because South surveyed \ans{2.5 times} as many students.
\textbf{The comparison rule:} to compare groups of different sizes, use \ans{relative frequencies},
never raw counts. But ``how many students in this district walk?'' asks about \ans{how many},
and there a count is exactly the right number --- \emph{say which number you used, and why}.
\notesprompt{Compare the schools.}
\begin{probgrid}{|Y|Y|}
\pcell{14}{Work both walking rates.}{1.3cm}{North $80/400 = 20\%$; South $150/1000 = 15\%$} &
\pcell{15}{Which school has more walkers? Which has the higher walking rate?}{1.3cm}{South, 150 to 80; North, 20\% to 15\%} \\ \hline
\pcell{16}{So which school ``walks more''? Answer honestly.}{1.2cm}{It depends on the question --- both are true} &
\pcell{17}{What one change would make the district's chart a fair comparison?}{1.2cm}{Relabel the axis as the percent of each school} \\ \hline
\pcell{18}{``How many students in this district walk to school?'' Which number, and why?}{1.3cm}{A count, $80 + 150 = 230$: the question asks how many} &
\pcell{19}{A student divides South's 150 walkers by 1{,}400. What did she compute?}{1.3cm}{150 as a share of both schools --- the wrong total} \\ \hline
\end{probgrid}
\\ \hline
...
\begin{probgrid}{|Y|Y|Y|}
\pcell{20}{Which shift sold more large drinks?}{1.0cm}{Morning, 84 to 32} &
\pcell{21}{Morning's large-drink rate.}{1.0cm}{$84/240 = 35\%$} &
\pcell{22}{Evening's large-drink rate.}{1.0cm}{$32/80 = 40\%$} \\ \hline
\pcell{23}{Which shift sells large drinks at the higher rate? Is that the same question as
problem~20?}{1.4cm}{Evening, 40\% to 35\%; no --- more vs.\ more often} &
\pcell{24}{One sentence for the staff newsletter comparing what the shifts sell. Which kind of
number did you use, and why?}{1.4cm}{A rate: evening 40\% vs.\ 35\%, since 80 vs.\ 240 orders} &
\pcell{25}{The owner asks, ``Which shift is \emph{busier}?'' Which column answers that --- and
is the answer a rate or a count?}{1.4cm}{The Total column: morning, 240 to 80 --- a count} \\ \hline
\end{probgrid}
\\ \hline
\end{guidednotes}
