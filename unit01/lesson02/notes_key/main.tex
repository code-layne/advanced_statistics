...
\begin{vocabbox}
\small
\vocabans{Variable}{A characteristic that can take different values for different individuals}
\vocabans{Categorical variable}{Places an individual into one of several groups or categories; no meaningful arithmetic on the values}
\vocabans{Quantitative variable}{Takes numerical values for which arithmetic (average, difference) makes sense}
\vocabans{Discrete quantitative variable}{Countable values with gaps between them (e.g., number of siblings: 0, 1, 2, \ldots)}
\vocabans{Continuous quantitative variable}{Can take any value in an interval; measured, not counted (e.g., height, time)}
\vocabans{Distribution of a variable}{Describes what values the variable takes and how often each value occurs}
\end{vocabbox}
...
\vspace{6pt}
\renewcommand{\arraystretch}{1.5}
\begin{tabularx}{\linewidth}{@{} >{\centering\arraybackslash\columncolor{navy}\color{white}}p{0.28\linewidth}
                                    >{\centering\arraybackslash}X
                                    >{\centering\arraybackslash}X @{}}
\textcolor{white}{\textbf{Question}} &
\textcolor{navy}{\textbf{Discrete}} &
\textcolor{navy}{\textbf{Continuous}} \\
\textcolor{white}{Can I list all possible values?} & \ans{Yes} & \ans{No} \\
\textcolor{white}{Is it measured on a scale?}      & \ans{No}  & \ans{Yes} \\
\textcolor{white}{Are there gaps between values?}  & \ans{Yes} & \ans{No} \\
\end{tabularx}
\end{notesbox}

\vspace{0.1in}

% ── DECISION FLOWCHART — printed identically in the blank; the key must carry it
% too or the key runs a page short against the blank.
\boxguard[30]
