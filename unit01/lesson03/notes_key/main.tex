...
\begin{vocabbox}
\small
\vocabans{Frequency}{The count of individuals in a particular category}
\vocabans{Relative frequency}{The frequency divided by the total number of individuals; expressed as a proportion (decimal) or percent}
\vocabans{Frequency table}{A table listing each category of a categorical variable and the count of individuals in that category}
\vocabans{Relative frequency table}{A table listing each category and the proportion (or percent) of individuals in that category}
\vocabans{Distribution (of a categorical variable)}{Describes all possible values a categorical variable takes and how often each value occurs (given as relative frequencies)}
\end{vocabbox}
...
\begin{enumerate}[leftmargin=1.5em, itemsep=6pt]
  ...
  \item Percentage \textit{not} by bus: \ans{$1 - 0.32 = 0.68 = 68\%$}
        (or $(10+4+3)/25 = 17/25$)
  \item \ans{Of the 25 Statistics students surveyed, the most common mode of
