...
\begin{tcolorbox}[colback=sky, colframe=navy, boxrule=0.9pt, arc=2mm,
    ...
    Students construct only by filling in the interval table (notes problem~9). Do not add a
...
\begin{minipage}[t]{0.48\linewidth}
    \begin{itemize}
        ...
        \item \textbf{Cumulative graph} --- how many observations are at or below a value
              (EK UNC-1.G.4) --- named in the Histograms row, worked in Lesson 1.6
...
Guided Notes \& Practice & 34 & Fill the vocabulary box and the notes table: problems 1--15 row by row, then \emph{The Bus Stop} (16--19) together, pen in their hand & \textbf{I do} each row's definition, its picture, and its first problem, \textbf{we do} the rest of its grid (24) $\to$ \textbf{we do} Guided Practice (10) \\
...
The notes are one \emph{Main Ideas / Notes} table --- five rows, 19 problems with room to write. \textbf{In
every row you read the definition, mark up the picture, and work the first problem of the grid; the class works
the rest, pen in their hand.} The vocabulary box is filled \emph{as you go}, never in advance.
\begin{multicols}{2}
\textbf{Discrete \& Continuous (5 min).} Hang the two definitions on the warm-up sentence still
on the board, then the test: ask about the variable, not the digits. Work problem~1 yourself;
problems 2--4 are theirs, and \textbf{problem~3 is the rounding trap} --- a gauge reading whole
millimetres has not made rainfall discrete. Take a hand count on it before anyone speaks.

\textbf{Every Value (5 min).} The stem plot and the dotplot of the same 24 times, side by side.
The point is not the mechanics but that \emph{two displays that look nothing alike carry
identical information}, because both keep every value. Name both displays in their boxes, then problems 5--8 are read
straight off them; make them find the two 36s in each (problem~6), and let problem~8 say why either
display rebuilds the other.

\textbf{Histograms (6 min).} The three steps, then \textbf{problem~9 is the hinge}: they fill
the interval table themselves, and then you say the line that makes the histogram cheap ---
\textbf{that table is already the histogram}. Step~3's \emph{the bars touch} takes the contrast straight
back to Lesson 1.2's gaps; name the cumulative graph in one sentence without working it.

\textbf{Drawn Twice (8 min) --- the crux.} Put both pictures up before computing anything and
ask which one is lying. Problem~12 first, together --- both add to 24 --- so that ``data went
missing'' is off the table before the argument starts. Problems 13--14 land the sentence
\emph{a histogram is a choice, not a fact}; say it, have them restate it in the \emph{In your own words} space, and point at the
two pictures while you do. \textbf{Problem~15 is the counterweight}: blocks of one minute give
every value back and no shape at all. Do not skip it.

\columnbreak
\textbf{We do (about 10 min): The Bus Stop.} A third display in a fresh context, problems
16--19, worked entirely together. Students hold the pen; you hold the questions: \emph{``What
makes those two riders look strange --- where they are, or what is next to them?''} \quad
\emph{``Count me the 5--9 block. Is it empty?''} \textbf{Problem~18 is the crux transferred}
and is the one to protect when time is short.
...
on \emph{discrete or continuous} in problem~17 before anyone speaks, and make the room defend
both answers.

Circulate and demand the reason, not the letter:
\begin{itemize}[leftmargin=1.1em, nosep]
  \item \textbf{Problem~16:} ``Longer than 8 minutes --- read me the dots you counted, not the
        number.''
  \item \textbf{Problem~17, the rounding trap:} ``The picture only shows whole minutes. Did the
        bus care?''
  \item \textbf{Problem~18, the crux:} ``Count me the 5--9 block. Then what happens to the two
        riders out at 11 and 12?''
\end{itemize}
While circulating, pick the papers to put up in the debrief --- one whose problem~18 kept the
two riders but lost the empty stretch, one that missed that the stretch went with them.
...
\begin{enumerate}[leftmargin=1.3em, nosep, itemsep=3pt]
  ...
  \item Put up the two Guided Practice papers you picked and name what the correct one
        did. Problem~18 regroups in the \emph{other} direction from the notes, so this is where
        you find out whether students learned the rule or learned ``wide is bad.''
  \item Circle the warm-up sentence still on the board and take the rounding trap off problem~17:
...
a printed definition, a big picture, and a few problems with room to write. \textbf{In every
row you read the definition, mark up the picture, and work the first problem; the class works the rest with the pen in their hand.} The first two
rows are machinery and must move --- five minutes each. The temptation is to teach the stem plot
carefully; resist it. The Every Value row exists to establish one thing --- both displays keep
every value --- and the two 36s (problem~6) prove it faster than any explanation.

\textbf{Problem~9 is the hinge, and the interval table is the reason it works.} Students fill
it in themselves, and then the histogram costs you one sentence: \emph{that table is already the
histogram}. If you draw the histogram first and the table second, the crux lands much harder,
because the width stops looking like a choice somebody made.

\textbf{Spend the time in Drawn Twice}, which carries the misconception. Put both pictures up
before computing anything and ask which one is lying. Then do problem~12 live --- both total
24 --- so nobody spends five minutes suspecting the data was tampered with. Do not resolve the
tension too fast: the sentence to land is that both are correct and they answer different
questions.

\textbf{Do not skip problem~15}, the counterweight (blocks of one minute). Without it students
leave with ``narrower is better,'' which is wrong, and it is the version of this error the AP
exam actually tests.

\textbf{The Bus Stop is where the pen changes hands.} It is a dotplot, so students meet the third
display in a working context, and problem~18 is the crux transferred: the two far-out riders
survive regrouping, but the empty stretch that made them look unusual does not. If you only get
through 21, 24, and 25, that is the right trade.
