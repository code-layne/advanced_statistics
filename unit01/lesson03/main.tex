...
\begin{tcolorbox}[colback=sky, colframe=navy, boxrule=0.9pt, arc=2mm,
    ...
    Students construct only by filling in the interval table (notes problem~13). Do not add a
...
Guided Notes \& Practice & 34 & Fill the vocabulary box and the notes table: problems 1--20 row by row, then \emph{The Bus Stop} (21--26) together, pen in their hand & \textbf{I do} each row's definition lines and first problem, \textbf{we do} the rest of its grid (24) $\to$ \textbf{we do} Guided Practice (10) \\
...
problems 2--6 are theirs, and \textbf{problem~4 is the rounding trap} --- a gauge reading whole
...
identical information}, because both keep every value. Problems 7--12 are read straight off the
displays; make them find the two 36s in each (problem~10), and let problem~12 say why either
display rebuilds the other.

\textbf{Histograms (6 min).} The three steps, then \textbf{problem~13 is the hinge}: they fill
the interval table themselves, and then you say the line that makes the histogram cheap ---
\textbf{that table is already the histogram}. Problem~15 takes the bars-touch contrast straight
...
ask which one is lying. Problem~17 first, together --- both add to 24 --- so that ``data went
missing'' is off the table before the argument starts. Problems 18--19 land the sentence
\emph{a histogram is a choice, not a fact}; say it, write it in the callout, and point at the
two pictures while you do. \textbf{Problem~20 is the counterweight}: blocks of one minute give
...
21--26, worked entirely together. Students hold the pen; you hold the questions: \emph{``What
makes those two riders look strange --- where they are, or what is next to them?''} \quad
\emph{``Count me the 5--9 block. Is it empty?''} \textbf{Problem~25 is the crux transferred}
...
on \emph{discrete or continuous} in problem~23 before anyone speaks, and make the room defend
both answers.

Circulate and demand the reason, not the letter:
\begin{itemize}[leftmargin=1.1em, nosep]
  \item \textbf{Problem~22:} ``Longer than 8 minutes --- read me the dots you counted, not the
        number.''
  \item \textbf{Problem~23, the rounding trap:} ``The picture only shows whole minutes. Did the
        bus care?''
  \item \textbf{Problem~25, the crux:} ``Count me the 5--9 block. Then what happens to the two
        riders out at 11 and 12?''
\end{itemize}
While circulating, pick the papers to put up in the debrief --- one whose problem~25 kept the
...
\begin{enumerate}[leftmargin=1.3em, nosep, itemsep=3pt]
  ...
  \item Put up the two Guided Practice papers you picked and name what the correct one
        did. Problem~25 regroups in the \emph{other} direction from the notes, so this is where
        you find out whether students learned the rule or learned ``wide is bad.''
  \item Circle the warm-up sentence still on the board and take the rounding trap off problem~23:
...
every value --- and the two 36s (problem~10) prove it faster than any explanation.

\textbf{Problem~13 is the hinge, and the interval table is the reason it works.} Students fill
...
before computing anything and ask which one is lying. Then do problem~17 live --- both total
...
\textbf{Do not skip problem~20}, the counterweight (blocks of one minute). Without it students
...
display in a working context, and problem~25 is the crux transferred: the two far-out riders
