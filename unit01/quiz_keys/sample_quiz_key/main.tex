%! Unit 1, Lessons 1.0–1.4 — SAMPLE Quiz (Answer Key)
...
% NAMESTRIP: no \namedateperiod — mirrors the blank.
% NO teachernote here — scoring guidance belongs in the teacher's copy, never in a key.
% GENERATED FROM the blank. PAGE LOCKSTEP with ../../quiz_keys/sample_quiz_key/: every \writelines{n} here is answered by
...
\pageheader{Unit 1 \quad Lessons 1.0--1.4}{Sample Quiz --- Answer Key}
...
\begin{enumerate}[leftmargin=1.9em, itemsep=0.13in]
  ...
  \item Which of the following is a \textbf{statistical question} about these vendors?
        \begin{enumerate}[label=\textbf{(\Alph*)}, leftmargin=2.2em, itemsep=2pt]
          ...
          \item \ans{How much does weekly revenue vary among the market's 48 vendors?}
  ...
  \item \textbf{Stall number} is best classified as
        \begin{enumerate}[label=\textbf{(\Alph*)}, leftmargin=2.2em, itemsep=2pt]
          ...
          \item \ans{categorical, because the number labels a stall rather than recording an amount.}
  ...
  \item Bayview Market has 30 vendors, 15 of them selling produce. Which statement about the
        ...
        \begin{enumerate}[label=\textbf{(\Alph*)}, leftmargin=2.2em, itemsep=2pt]
          ...
          \item \ans{Bayview is more produce-focused, because 50\% is greater than 37.5\%.}
  ...
  \item \textbf{Pounds of tomatoes sold in a week}, recorded to the nearest whole pound, is
        \begin{enumerate}[label=\textbf{(\Alph*)}, leftmargin=2.2em, itemsep=2pt]
          ...
          \item \ans{continuous, because between any two weights another weight is possible.}
...
\begin{enumerate}[leftmargin=1.9em, itemsep=0.16in, resume]
  ...
  \item Use the Riverside Farmers Market records above.
        ...
        \ansline{Individuals: the 48 vendors, one per row.}\\
        \ansline{Categorical: stall type. Quantitative: weekly revenue, in dollars.}

        \vspace{6pt}
        \textbf{(b)} Give the relative frequency of each of the four stall types, to the nearest
        tenth of a percent.\\[4pt]
        \ansline{Produce 37.5\%, Baked 25.0\%, Crafts 20.8\%, Prepared food 16.7\%.}
        ...
        \ansline{Different sizes, so the counts cannot be compared: 48 vendors give}\\
        \ansline{Riverside more of everything. Shares are fair: 18/48 = 37.5\% against}\\
        \ansline{15/30 = 50.0\%, so a share comparison favours Bayview.}
        ...
        \ansline{``How much does weekly revenue vary among the 48 vendors?''}\\
  ...
  \item The dotplot below shows the \textbf{weekly revenue}, in dollars, of all 30 vendors at a
        ...
        \ansline{Continuous --- revenue is a measured amount of money.}\\
        \ansline{Between \$480 and \$481, \$480.25 is possible; rounding changes nothing.}

        \vspace{6pt}
        \textbf{(b)} Describe the distribution of weekly revenue completely.\\[4pt]
        \ansline{Skewed right: the dots pile up near \$300--\$500, tail out to the right.}\\
        \ansline{A gap from about \$1,000 to \$1,900, and one outlier near \$1,900.}\\
        \ansline{Center about \$500 per week; values run from about \$200 to \$1,900 ---}\\
        \ansline{a great deal of variability in weekly revenue among the 30 vendors.}
        ...
        \ansline{side the data piles up on. This tail reaches high, so: skewed right.}
        ...
        \ansline{Nothing marks it as an error --- a stall can have a big week, and an}\\
        \ansline{outlier is a finding. Investigate it and report it in the description.}
