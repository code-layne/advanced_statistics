...
% row by row through the Main Ideas / Notes table -> we do (guided practice). THERE IS NO
% INDEPENDENT PRACTICE SET — the homework is the individual practice, and the period ends with
% students starting it. Teacher-facing. Every box is
...
\begin{tcolorbox}[colback=sky, colframe=navy, boxrule=0.9pt, arc=2mm,
    ...
    idea a printed definition, a picture to mark up, and a grid of short problems, the first worked by you and the
...
\begin{itemize}[leftmargin=1.1em, nosep, after=\vspace{2pt}]
  ...
  \item \textbf{Target misconception:} that the mean is simply ``the right answer'' for center
        ...
        crux that surfaces it is problem~11 in the notes' \emph{Resistance} row --- ``the median
        is more accurate.''
...
Guided Notes \& Practice & 34 & Fill the vocabulary box and the notes table: problems 1--12 row by row, then \emph{Quiz Scores} (13--17) together, pen in their hand & \textbf{I do} each row's definition, picture mark-up and first problem, \textbf{we do} the rest of its grid (24) $\to$ \textbf{we do} Guided Practice (10) \\
...
\begin{minipage}[t]{0.48\linewidth}
    ...
    \begin{itemize}
        ...
        \item \textbf{3.} Split an ordered list of seven, then split each half again.
              ...
              the Quartiles row names it ten minutes later.
    \end{itemize}
\end{minipage}
\hfill
\begin{minipage}[t]{0.48\linewidth}
    ...
    \begin{itemize}
        \item \textbf{Debrief item~3 aloud} and \textbf{leave the board work up all period}:
              ...
              middle $|$ upper half), 5, 9, and 15 circled. The Quartiles row attaches $Q_1$, the
...
The notes are one \emph{Main Ideas / Notes} table --- five rows, 17 short problems. \textbf{In
every row you read the definition, mark up the picture, and work the first problem of the grid;
the class works the rest, pen in their hand.} The vocabulary box is filled \emph{as you go}, never in advance.
\begin{multicols}{2}
\textbf{Center (6 min).} Read the definition, then mark up the paycheck dotplot: the arrow to
the middle dot is the \emph{median} (\$750), the triangle where the dots would balance is the
\emph{mean} (\$1{,}000) --- fill both label boxes. Work problem~1 yourself: make a student
order the nine before you circle the fifth --- the list is printed out of order on purpose.
Problem~2 is theirs, and \textbf{problem~2 is where you withhold the verdict}: nobody takes home
\$1{,}000, yet the owner's ``we average \$1{,}000'' is the mean and it is not wrong. Say out
loud that both numbers are correct and that the Resistance row decides which one to report.

\textbf{Quartiles (5 min).} Read the definition, then Route A: the split move from the warm-up
board with its names attached. Work problem~3 (the even-count median) yourself and label the
middle dashed cut on the dotplot; problems 4--6 are theirs, and the $Q_1$ and $Q_3$ label boxes
get filled as 4 and~5 land. Percentile gets the one sentence on the page and its interpretation
in context --- \emph{about 75\% took 31.5 minutes or less} --- and no more.

\textbf{Spread (6 min).} Read the definition, then the deviation picture on the five-package
route and the only hand computation of $s_x$ this course asks for, in the \texttt{work} block:
squares, $\sqrt{10}$, then say plainly that the calculator does this from now on. Work
problem~7 yourself --- say \emph{one number, not an interval} while you write $33 - 27$ ---
and problem~8 is theirs. The interval trap waits in the Resistance grid (problem~10).

\textbf{Resistance (7 min) --- the crux.} Read the definition, then the before/after picture:
the \$3{,}000 becomes \$750; fill the two label boxes (\$750 and \$750) and write \$1{,}000
$\to$ \$750 beside \$750 $\to$ \$750 on the board; the \emph{In your own words} line is
where someone says \emph{why} one of them did not care. Then Routes B and C: fill Route C's row
of the table and let the room see range 24 = range 24 on the strength of one package while the
IQR stays 3. Work problem~9 yourself (the IQR tells the routes apart); \textbf{problem~10 is the
interval trap} --- ``27 to 33'' is not a range; \textbf{problem~11 is the crux} --- take a hand
count on ``more accurate'' before anyone speaks, then replace \emph{accurate} with
\emph{resistant} on the board; problem~12 applies the reporting rule in both directions, so
nobody leaves believing the median is always better.

\columnbreak
\textbf{We do (about 10 min): Quiz Scores.} Eleven scores in a fresh context, problems 13--17,
worked entirely together --- eleven so the median is a single value and each half holds exactly
five. Students hold the pen; you hold the questions: \emph{``Which position in the list did you
change? Is that position anywhere near the middle?''} \quad \emph{``The IQR only looks at two
numbers. Did either of them change?''} \textbf{Problems 16--17 are the crux transferred} and the
ones to protect when time is short; problem~17 ends on Skill 4.B in one sentence.

\textbf{The pen must actually change hands.} Put the five-number table (problem~13) on them cold,
take a hand count on \emph{does the median move?} in problem~16 before anyone speaks, and make
the room defend both answers before the arithmetic settles it.

Circulate and demand the reason, not the verdict:
\begin{itemize}[leftmargin=1.1em, nosep]
  \item \textbf{Problem~13:} ``Read me the five below the median. Which one is their middle?''
  \item \textbf{Problem~15, the range as one number:} ``I asked for one number. Which
        subtraction gives it?''
  \item \textbf{Problem~16, the crux transferred:} ``Four verdicts is not an answer. What rule
        sorts them into two piles?''
\end{itemize}
While circulating, pick the papers to put up in the debrief --- one whose problem~16 says the
...
\begin{enumerate}[leftmargin=1.3em, nosep, itemsep=3pt]
  \item \textbf{Open on the almost-right answer.} Put up the Guided Practice paper whose
        problem~16 says \emph{``the median is more accurate.''} It is not more accurate --- the mean of the
  ...
  \item Take the interval trap off problems 10 and~15: circle a ``7'' and a ``12 to 19'' and ask which one
...
\begin{itemize}[leftmargin=1.2em, nosep]
  \item \textbf{``The mean is the real answer; the median is a shortcut''} (notes~2, 11--12,
        and~16--17, HW~1, AP~MC1 and~(b) --- the target misconception). Probe: \emph{``Name the
  ...
  \item \textbf{Median found without ordering the list} (notes Center row, HW~1). The paycheck list is
        printed out of order on purpose. Probe: \emph{``Read me your list from smallest to
        largest first.''}
  \item \textbf{Range reported as an interval} --- ``the range is 27 to 51'' (notes~10 and~15,
        HW~2). Probe: \emph{``I asked for one number. Which subtraction gives it?''}
  \item \textbf{Even-count median taken as one of the two middle values} (notes Quartiles row, HW~3): with
        twelve times they must average. Probe: \emph{``Six below and six above --- so which
        single one is in the middle? Neither. What now?''}
  \item \textbf{IQR computed as $Q_1$ to $Q_3$, or the halves split wrongly} (notes~4--5 and~13,
        HW~3). Probe: \emph{``Go back to the warm-up on the board. How many numbers were in each
        half?''}
  \item \textbf{Standard deviation assumed resistant} (notes Spread row, AP~MC2, HW~5): it is built from
...
\textbf{item~2 is the column that tells you whether the Resistance row landed} --- five values, one
...
Resistance row if that third pile ran past a third of the room. Then close on what
...
twice --- once when the Quartiles row names $Q_1$ and $Q_3$ on Route A, and once at problem~14,
the Guided Practice table. Use the students' own words here (``the middle one,'' ``the middle of each
...
a printed definition, a picture to mark up, and a grid of short problems. \textbf{In every row
you read the definition, mark up the picture, and work the first problem; the class works the
rest with the pen in their hand.} The first two rows are machinery and must move --- six minutes
on Center, five on Quartiles. Label the median and the mean on the paycheck dotplot, and make a
student order the nine before you circle the fifth (problem~1) --- the list is printed out of
order on purpose. The line to land in Center is that nobody earns \$1{,}000, and the line to
\emph{withhold} is which number to report --- both live in problem~2. Say plainly that the
Resistance row decides it.

\textbf{Spread is where the range becomes one number.} Write $33 - 27$ and say ``six --- not
twenty-seven to thirty-three'' before anyone writes an interval, and let problem~10, in the
Resistance grid, catch whoever does. The standard deviation gets one hand computation, the formula, and the sentence \emph{the
calculator does this from now on}; do not let it expand past six minutes, because nothing in
this packet asks for a second one by hand.

\textbf{Spend the time in Resistance}, which carries the misconception. Swap the \$3{,}000 for
\$750 on the before/after picture, fill its two label boxes, and write the two arrows side by
side before saying \emph{resistant}. Then fill Route C's row of the table and let the room see
24 against 24 --- the range cannot tell Route C from Route B --- before you point at the IQR
column (problem~9). Problem~10 is the interval trap. \textbf{Problem~11 is the crux}: ``more
accurate'' is the almost-right answer, and the word to replace it with is \emph{resistant}. Do
not resolve the tension too fast: the sentence to land is that both measures of center are
correct and the shape picks --- problem~12 applies that rule in both directions.

\textbf{Quiz Scores is where the pen changes hands.} Eleven values so the table (problem~13) is
clean; problems 16--17 are the crux transferred, and 17 ends on the reporting rule in one
sentence. If you only get through 13, 16, and~17, that is the right trade.
...
Resistance row}, and say so plainly.
