...
Guided Notes \& Practice & 34 & Fill the vocabulary box and the notes table: problems 1--13 row by row, then \emph{Quiz Scores} (14--20) together, pen in their hand & \textbf{I do} each row's definition lines and first problem, \textbf{we do} the rest of its grid (24) $\to$ \textbf{we do} Guided Practice (10) \\
...
\begin{multicols}{2}
\textbf{Center (6 min).} \emph{Statistic} in one line, then the nine paychecks: the mean in the
\texttt{work} block (\$9{,}000 over 9), then make a student order the nine before you take the
fifth --- the list is printed out of order on purpose. Work problem~1 yourself (nobody takes home
\$1{,}000); problems 2--3 are theirs, and \textbf{problem~3 is where you withhold the verdict}:
the owner's ``we average \$1{,}000'' is the mean, and it is not wrong. Say out loud that both
numbers are correct and that the Resistance row decides which one to report.

\textbf{Quartiles (5 min).} Route A: twelve times, the even-count median, and the split move from
the warm-up board with its names attached. Work problem~4 ($Q_1 = 28.5$) yourself, pointing at the
board; problems 5--6 are theirs. Percentile gets the one sentence on the page and its
interpretation in context --- \emph{about 75\% took 31.5 minutes or less} --- and no more.

\textbf{Spread (6 min).} Range and IQR in the \texttt{work} block --- say \emph{one number, not an
interval} while you write $33 - 27$ --- then the standard deviation on the five-package route, the
only hand computation of $s_x$ this course asks for: deviations, squares, $\sqrt{10}$, then say
plainly that the calculator does this from now on. Work problem~7 yourself; \textbf{problem~8 is
the interval trap} --- ``27 to 33'' is not a range --- and problem~9 is units: re-express in
seconds and every statistic is multiplied by 60.

\textbf{Resistance (7 min) --- the crux.} Swap the \$3{,}000 for \$750 in the \texttt{work} block
and write \$1{,}000 $\to$ \$750 beside \$750 $\to$ \$750; make someone say \emph{why} one of them
did not care. Then Routes B and C: fill the table and let the room see range 24 = range 24 on the
strength of one package while the IQR stays 3. Land \emph{resistant} and \emph{nonresistant} on
that table, then the reporting rule and the two outlier rules in one breath. Work problem~10
yourself (the IQR tells the routes apart); \textbf{problem~11 is the crux} --- take a hand count
on ``more accurate'' before anyone speaks, then replace \emph{accurate} with \emph{resistant} on
the board; problems 12--13 apply the reporting rule in both directions, so nobody leaves
believing the median is always better.

\columnbreak
\textbf{We do (about 10 min): Quiz Scores.} Eleven scores in a fresh context, problems 14--20,
...
numbers. Did either of them change?''} \textbf{Problems 18--19 are the crux transferred} and the
ones to protect when time is short; problem~20 is Skill 4.B in one sentence.

\textbf{The pen must actually change hands.} Put the five-number table (problem~14) on them cold,
take a hand count on \emph{does the median move?} in problem~18 before anyone speaks, and make
the room defend both answers before the arithmetic settles it.

Circulate and demand the reason, not the verdict:
\begin{itemize}[leftmargin=1.1em, nosep]
  \item \textbf{Problem~14:} ``Read me the five below the median. Which one is their middle?''
  \item \textbf{Problem~16, the interval trap:} ``I asked for one number. Which subtraction gives
        it?''
  \item \textbf{Problem~19, the crux:} ``Four verdicts is not an answer. What rule sorts them
        into two piles?''
\end{itemize}
While circulating, pick the papers to put up in the debrief --- one whose problem~19 says the
...
\begin{enumerate}[leftmargin=1.3em, nosep, itemsep=3pt]
  \item \textbf{Open on the almost-right answer.} Put up the Guided Practice paper whose
        problem~19 says \emph{``the median is more accurate.''} It is not more accurate --- the mean of the
  ...
  \item Take the interval trap off problem~16: circle a ``7'' and a ``12 to 19'' and ask which one
...
\begin{itemize}[leftmargin=1.2em, nosep]
  \item \textbf{``The mean is the real answer; the median is a shortcut''} (notes~3, 11--13,
        and~19--20, HW~1, AP~MC1 and~(b) --- the target misconception). Probe: \emph{``Name the
  ...
  \item \textbf{Median found without ordering the list} (notes Center row, HW~1). The paycheck list is
        ...
        largest first.''}
  \item \textbf{Range reported as an interval} --- ``the range is 27 to 51'' (notes~8 and~16,
  ...
  \item \textbf{IQR computed as $Q_1$ to $Q_3$, or the halves split wrongly} (notes~4--5 and~14,
...
and work the first problem; the class works the rest with the pen in their hand.} The first two
rows are machinery and must move --- six minutes on Center, five on Quartiles. Compute the mean
in the \texttt{work} block live and make a student order the nine before you take the fifth ---
the list is printed out of order on purpose. The line to land in Center is that nobody earns
\$1{,}000 (problem~1); the line to \emph{withhold} is which number to report (problem~3). Say
plainly that the Resistance row decides it.

\textbf{Spread is where the range becomes one number.} Write $33 - 27$ and say ``six --- not
twenty-seven to thirty-three'' before anyone writes an interval, and let problem~8 catch whoever
does. The standard deviation gets one hand computation, the formula, and the sentence \emph{the
...
\$750 in the \texttt{work} block and write the two arrows side by side before saying
\emph{resistant}. Then fill the routes table and let the room see 24 against 24 --- the range
cannot tell Route C from Route B --- before you point at the IQR column (problem~10).
\textbf{Problem~11 is the crux}: ``more accurate'' is the almost-right answer, and the word to
replace it with is \emph{resistant}. Do not resolve the tension too fast: the sentence to land is
that both measures of center are correct and the shape picks --- problems 12 and~13 apply that
rule in both directions.

\textbf{Quiz Scores is where the pen changes hands.} Eleven values so the table (problem~14) is
clean; problems 18--19 are the crux transferred and problem~20 is the reporting rule in one
sentence. If you only get through 14, 18, and~19, that is the right trade.
