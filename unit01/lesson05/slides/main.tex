...
\begin{frame}[t, plain]
  ...
  \sectionlabel{notes section 1 $\cdot$ a statistic is a numerical summary of sample data}
  \vspace{4pt}
  \begin{columns}[t]
  ...
  \begin{center}
    ...
    {\small\color{slate} Which one to \emph{report} is the question section 2 settles.}
  \end{center}
\end{frame}
...
\begin{frame}[t, plain]
  ...
  \sectionlabel{notes section 1 $\cdot$ where does a value sit in the pack?}
  \vspace{4pt}
...
\begin{frame}[t, plain]
  ...
  \sectionlabel{notes section 2 $\cdot$ spread, as a single number}
  \vspace{4pt}
  \begin{itemize}\setlength{\itemsep}{7pt}
...
\begin{frame}[t, plain]
  ...
  \begin{itemize}\setlength{\itemsep}{5pt}
    ...
    \item Compute the range and the IQR --- one number each.
    \item Then the lowest score turns out to be a \textbf{1}, not a 12. Which of \emph{mean,
          median, range, IQR} move? Which pair would you report now?
  \end{itemize}
