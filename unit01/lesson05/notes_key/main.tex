...
\small
\begin{multicols}{2}
\vocabans{Dotplot}{Each data value is a dot above its position on a number line. Reveals every individual value; best for small datasets.}
\vocabans{Stemplot (stem-and-leaf plot)}{Separates each value into a stem (leading digits) and a leaf (trailing digit). Preserves actual values and sorts them.}
\columnbreak
\vocabans{Histogram}{Groups quantitative data into equal-width bins; bar height is frequency or relative frequency. Bars touch.}
\vocabans{Bin (class interval)}{An equal-width grouping on the horizontal axis. e.g., $[20,40)$ means 20 up to but not including 40.}
\vocabans{Gap / Cluster}{A gap is a range with no observations. A cluster is a concentration of observations in a narrow range.}
\end{multicols}
\end{vocabbox}
