...
\begin{enumerate}[leftmargin=1.9em, itemsep=6pt]
  \item Seven employees at a small office reported their commute times, in minutes:
        ...
        \textbf{(a)} Mean: \ans{22 minutes} \qquad Median: \ans{16 minutes}
        ...
        \answerspace{2.8cm}{The median, 16 minutes. The distribution is skewed to the right: six
        of the seven commutes fall between 12 and 19 minutes, and the single 60-minute commute
        drags the mean up to 22 --- longer than every commute in the office except that one. The
        median is resistant, so it still sits inside the group of six.}
  \item Five children reported how many books they read over the summer:
        ...
        \vspace{4pt}
        New mean: \ans{14} \qquad New median: \ans{10} \qquad New range:
        \ans{24}

        \vspace{5pt}
        In one or two sentences, say which summaries moved, which did not, and why.
        \answerspace{2.6cm}{The mean moved from 10 to 14 and the range from 4 to 24; the median
        did not move at all. The mean and the range are nonresistant --- both are computed from
        the changed value itself --- while the median depends only on which score is third in the
        ordered list, and that is still 10.}
  \item Twelve students recorded how many minutes they spent on homework one evening. The
        ...
        Median: \ans{57.5 min} \quad $Q_1$: \ans{35 min} \quad $Q_3$: \ans{80 min} \quad
        IQR: \ans{45 min}

        \vspace{5pt}
        Show how you split the list to get $Q_1$ and $Q_3$.
        \answerspace{2.4cm}{Twelve values, so six fall in each half and the median is halfway
  ...
  \item A pediatrician's growth chart reports that the \textbf{90th percentile} of birth weight
        ...
        context.
        \answerspace{2.4cm}{About 90\% of newborn girls weigh 3{,}900 grams or less at birth,
        and only about 10\% weigh more than that.}
  \item \textbf{AP-style multiple choice.} A company with 40 employees reports that the
        ...
        \end{enumerate}
        \answerspace{2.6cm}{(B). The mean is nonresistant, so a handful of very large salaries
        can pull it far above the median while the median stays with the middle employee --- and
        that is exactly what a right-skewed salary distribution looks like. (A) is wrong because
        it is the median, not the mean, that has half the employees above it.}
\end{enumerate}
\end{notesbox}
