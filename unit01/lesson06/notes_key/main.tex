...
The \textbf{five-number summary} of a quantitative variable is, in order, the \ans{minimum},
\ans{$Q_1$}, the \ans{median}, \ans{$Q_3$}, and the \ans{maximum}. A \textbf{boxplot} is
that summary \ans{drawn} to scale on a number line --- five numbers, nothing else.
...
\pcell{1}{Minimum}{0.9cm}{30} &
\pcell{2}{$Q_1$}{0.9cm}{34.5} &
\pcell{3}{Median}{0.9cm}{39.5} \\ \hline
\pcell{4}{$Q_3$}{0.9cm}{50.5} &
\pcell{5}{Maximum}{0.9cm}{74} &
\pcell{6}{Which part of the boxplot marks $Q_1$?}{1.3cm}{The left edge of the box} \\ \hline
\end{probgrid}
\\ \hline
% ── ROW 2: boxplot anatomy, as a procedure ───────────────────────────────────
\mainidea[Anatomy of a]{Boxplot} &
The \textbf{box} runs from \ans{$Q_1$} to \ans{$Q_3$} --- the middle \ans{50}\% of the
data --- and the line inside it is the \ans{median}. Each \textbf{whisker} reaches the most
extreme value that is \ans{not} an outlier. An outlier is not swallowed by a whisker: it
gets its \ans{own dot}, which makes the display a \textbf{modified boxplot} --- the version the
AP exam draws.

\vspace{3pt}
\stepnum{1}\ Mark the five numbers on a \ans{number line}.\par
\stepnum{2}\ Draw the box from $Q_1$ to $Q_3$, with a line at the \ans{median}.\par
\stepnum{3}\ Test each end with the \ans{$1.5 \times$ IQR} rule from Lesson~1.5.\par
\stepnum{4}\ Run each whisker out to the most extreme value that is \ans{not an outlier}; plot any
outlier on its own.
\notesprompt{Use the free-throw boxplot above.}
\begin{probgrid}{|Y|Y|Y|}
\pcell{7}{How wide is the box?}{1.2cm}{$50.5-34.5=16$ shots} &
\pcell{8}{How many of the twenty players sit inside the box?}{1.2cm}{10 --- half of them} &
\pcell{9}{Does this plot show an outlier? How can you tell?}{1.2cm}{No --- no point is drawn on its own} \\ \hline
\pcell{10}{Between which two totals does the top quarter sit?}{1.5cm}{50.5 and 74 shots} &
\pcell{11}{The lower whisker runs 30 to 34.5. What does its short length say?}{1.5cm}{The lowest five totals are packed close together} &
\pcell{12}{Name one thing the dotplot shows that the boxplot cannot.}{1.5cm}{Every individual total --- and the clump from 30 to 38} \\ \hline
\end{probgrid}
\\ \hline
...
box, upper half of the box, upper whisker. Each one holds \ans{25}\% of the data, however
wide it looks. Width reports \ans{spread} --- it never reports \ans{count}.
...
your counts to say what is wrong.}{1.5cm}{It holds five players, the same as every other section.} &
\pcell{15}{So what does that wide section actually tell you about those players?}{1.5cm}{Those five totals are spread far apart --- from 52 to 74 shots.} \\ \hline
\pcell{16}{Ten players made how many free throws or fewer, and which of the five numbers told
you?}{1.5cm}{39.5 or fewer --- the median splits the twenty in half.} &
\pcell{17}{The dotplot has a tight clump from 30 to 38. Why can the boxplot not show
it?}{1.5cm}{The boxplot draws only five numbers; everything between them is gone.} \\ \hline
\end{probgrid}
\\ \hline
...
Comparing distributions means addressing \ans{center}, \ans{variability}, and unusual features ---
all three, every time, each with a \ans{comparative} word (\emph{higher than}, \emph{more variable
than}) and each \ans{in context}. Describing one group and then describing the other is
\emph{not} a comparison and earns nothing.

\vspace{3pt}
Under skew the mean leaves the median: skewed right pulls the mean to the \ans{right} of the
median; skewed left pulls it to the \ans{left}.
\\
...
\begin{probgrid}{|Y|Y|}
\pcell{18}{How wide is the middle half of each route's times?}{1.4cm}{A: $26-21=5$ min \quad B: $32-20=12$ min} &
\pcell{19}{Which route is typically faster, and which is less predictable? Say it in
context.}{1.4cm}{Route A is faster --- median 23 min against 25 --- and far more predictable: middle half 5 min against 12.} \\ \hline
\pcell{20}{Route B's 58-minute morning is drawn on its own. Show why, and say where its whisker
stops.}{1.7cm}{IQR $=12$, so $32+1.5(12)=50$; $58>50$. The whisker stops at 40 min --- the most extreme time that is not an outlier.} &
\pcell{21}{Route B is skewed right. Is its mean above or below its median of 25 minutes?
Why?}{1.7cm}{Above 25 min --- the long right tail pulls the mean up.} \\ \hline
\end{probgrid}
\\ \hline
...
\begin{probgrid}{|Y|Y|}
\pcell{22}{Report each section's median, with units.}{1.2cm}{Period 2: 80 points \quad Period 5: 72 points} &
\pcell{23}{Report each section's IQR, with the subtraction.}{1.2cm}{P2: $86-74=12$ \quad P5: $84-68=16$} \\ \hline
\pcell{24}{Which section is skewed, and which way? How does the picture tell you?}{1.7cm}{Period 2 skews left (long lower whisker); Period 5 skews right (stretched upper half).} &
\pcell{25}{Does either section show an outlier? How can you tell from the display?}{1.7cm}{No --- neither display draws a point on its own.} \\ \hline
\end{probgrid}
\\
& \begin{probgrid}{|Y|}
\pcell{26}{\textbf{One paragraph, in context}, comparing the two sections: center, variability,
and unusual features --- every claim carrying a comparative word.}{2.0cm}{Period 2's scores were typically higher than Period 5's --- median 80 points against 72. Period 5's were more variable: a middle half 16 points wide against 12, and a wider overall span. Period 2 trails toward low scores while Period 5 trails toward high; neither section shows an outlier.} \\ \hline
\end{probgrid}
\\ \hline
\end{guidednotes}
