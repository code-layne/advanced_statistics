...
% rows 1-3 build and read ONE boxplot (1.8), row 4 and Guided Practice compare TWO (1.9).
...
\begin{tcolorbox}[colback=sky, colframe=navy, boxrule=0.9pt, arc=2mm,
    ...
    notes rows 1--3 and Topic 1.9 (compare two or more) in row 4 and Guided Practice. Protect
...
Warm-Up & 5 & Five numbers off an ordered list, a counting drill, and one $1.5\times$IQR check &
              ...
              period \\
Guided Notes \& Practice & 34 & Fill the vocabulary box and the notes table: problems 1--21 row
              by row, then \emph{Two Science Sections} (22--26) together, pen in their hand &
...
Debrief & 8 & Pens down, papers up --- answer aloud, nothing to fill in & Open on an
              \emph{almost}-right answer to problem~14, then the count row of problem~13 and the
              comparison standard \\
...
\begin{minipage}[t]{0.48\linewidth}
    ...
    \begin{itemize}
        ...
        \item \textbf{2.} Count twelve values into three stated stretches. \textbf{Seeds:}
              problem~13, the counting move the crux depends on, made mechanical first.
        \item \textbf{3.} One $1.5\times$IQR check with the arithmetic shown. \textbf{Seeds:}
              problem~20, the Route B rider at 58 minutes.
    \end{itemize}
\end{minipage}
\hfill
\begin{minipage}[t]{0.48\linewidth}
    ...
    \begin{itemize}
        ...
        \item Item~2 is a drill --- 60 seconds. It exists so that counting is not the obstacle
              in problem~13, where the count \emph{is} the point.
        \item \textbf{Leave item~3's arithmetic on the board for the rest of the period.}
              Problem~20 reuses it with different numbers, and a student who can look up the
...
The notes are one \emph{Main Ideas / Notes} table --- five rows, 26 short problems. \textbf{In
...
and the free-throw dotplot with its boxplot beneath it is the picture the first three rows all
point back at.
\begin{multicols}{2}
\textbf{The Five Numbers (6 min).} Point at the warm-up's five numbers still on the board and
name them: \emph{five-number summary}. Fill the definition lines, then the boxplot sentence ---
``five numbers, nothing else'' is the claim the whole lesson rests on. Work problem~1 yourself
(minimum 30, read off the leftmost dot); problems 2--6 are theirs. \textbf{Problem~6 is worth a
hand count} --- which part of the boxplot marks $Q_1$ --- because students who answer ``the line
in the middle'' have not yet accepted that the box has \emph{two} edges.

\textbf{Anatomy of a Boxplot (7 min).} Fill the box/whisker/outlier lines, then walk the four
\texttt{stepnum} steps aloud on the free-throw picture. Work problem~7 yourself ($50.5 - 34.5 =
16$); 8--12 are theirs. \textbf{Problem~8 is the quiet one that matters}: ten of the twenty sit
inside the box, and a student who says ``most of them'' is already showing you the misconception
the next row exists to break. Problem~12 must name the individual values.

\textbf{One Quarter (8 min) --- the crux row.} Fill the four-sections line and the 25\% line,
then release \textbf{problem~13, the count row}. Do not let anyone estimate: every boundary falls
in a gap, so the four counts are exactly 5, 5, 5, 5. Write them on the board with
\textbf{25\% \quad 25\% \quad 25\% \quad 25\%} above them. \textbf{Problem~14 is the trap} --- the
classmate who says the wide part is where most of the shots landed --- and \textbf{problem~15 is
the counterweight}: width reports spread, never count. Take a hand count on 14 before anyone
speaks; pick one \emph{almost}-right answer off a desk for the debrief. Problem~17 is the cost of
the box, and it returns in the AP Practice page's MC~4.

\columnbreak
\textbf{Comparing Distributions (5 min).} Fill the checklist line --- center, variability,
unusual features, comparative word, in context --- and say plainly that describing one group then
the other earns nothing. Work problem~18 yourself (Route A's middle half is 5 minutes, Route B's
is 12); 19--21 are theirs. \textbf{Problem~20 is where the warm-up arithmetic gets reused}:
$32 + 1.5(12) = 50$, and $58 > 50$, so the whisker stops at 40.

\textbf{We do (about 10 min): Two Science Sections.} Problems 22--26, worked entirely together,
...
box is longer? So which way does it trail off?''} \textbf{Problem~26 is the AP-scored form} --- one
paragraph, in context, with comparative language --- and it is the one that cannot be cut. If you
only get 22, 23, and~26, that is the right trade.

Circulate and demand the reason, not the number:
\begin{itemize}[leftmargin=1.1em, nosep]
  \item \textbf{Problem~14, the crux:} ``Count them. How many in the wide section? How many in
        the narrow one? Now say that out loud.''
  \item \textbf{Problem~25:} ``Where would an outlier be drawn if there were one?''
  \item \textbf{Problem~26:} ``Cross out every word in your paragraph that is not comparing.''
\end{itemize}
While circulating, pick the papers for the debrief --- one \emph{almost}-right problem~14, and
one problem~26 that describes twice instead of comparing once. \textbf{Your formative read comes
...
\begin{enumerate}[leftmargin=1.3em, nosep, itemsep=3pt]
  \item \textbf{Open on the \emph{almost}-right problem~14 you collected} --- typically ``the
        wide part is spread out, so it has fewer people in the middle.'' Half right, and the half
        that is wrong is the lesson. Put the count row from problem~13 beside it: \textbf{5 \quad
  ...
  \item Take problem~8 next: ten of twenty inside the box, ten outside. The box is not ``where
        the data is''; it is where the middle half is.
  \item Put up the two problem~26 paragraphs and name the difference out loud: one wrote two
        descriptions, one wrote a comparison. \textbf{Only the second scores}, however accurate
        the first is.
  \item Close on the cost, off problem~17: gaps, clusters, and a second peak all vanish inside
...
position rule here --- right-skew puts the mean right of the median (problem~21). One line is
...
\begin{itemize}[leftmargin=1.2em, nosep]
  \item \textbf{``Most of the players are in the wide part''} (notes~14, 15; AP MC~1; HW~2, HW~5
        --- the target misconception). Probe: \emph{``Count them. How many dots in the wide
        section? Now the narrow one. Say that out loud.''}
  \item \textbf{Estimating the counts instead of counting} (notes~13). The crux only lands if the
        four numbers come out exactly 5, 5, 5, 5. Probe: \emph{``Point at each dot as you say the
        number.''}
  \item \textbf{Reading the box as ``where the data is'' and the whiskers as empty} (notes~8,
        AP MC~1). Probe: \emph{``How many of the twenty are past the right edge of the box?''}
  \item \textbf{Two descriptions parked side by side} (notes~19, 26; HW~4 --- the second
        misconception). Probe: \emph{``I read your sentence about Period 2. Now cross out every
        word that is not comparing.''}
  \item \textbf{Comparison with no context} (notes~26, HW~4): ``the median is higher'' with no
        variable, no units, no groups. Probe: \emph{``Higher what? Higher than what?''}
  \item \textbf{The lone dot read as an error in the data} (notes~20). It is a real rider with a
        real time. Probe: \emph{``Did somebody mistype it, or did somebody have a bad morning?''}
  \item \textbf{Shape claimed from a boxplot} (notes~17, AP MC~4, HW~5): a boxplot cannot show a
...
that counting is not the obstacle in problem~13. \textbf{Put item~3's four lines of arithmetic on
the board and leave them there for the whole period}: problem~20 runs the same rule on Route B's
58-minute morning, and the AP Practice free response runs it a third time on a \$158 shift.
\end{teachernote}
...
the homework start.} The notes are a \emph{Main Ideas / Notes} table, not prose: five rows, 26
short problems, and \textbf{in every row you fill the definition lines and work the first problem
while the class watches, then the class works the rest with the pen in their hand.} Rows 1--3 are
Topic 1.8 and all three read the same free-throw picture, so the picture is worth 30 seconds of
orientation before row~1: twenty dots on top, the boxplot built from them underneath.

\textbf{Spend the time in One Quarter, the crux row.} Problem~13 is the whole lesson: twenty dots
...
\textbf{problem~14 is the trap} (the classmate who reads width as count) and \textbf{problem~15
is the counterweight} (width reports spread). Take a hand count on 14 before anyone speaks and
pick one \emph{almost}-right answer off a desk --- the debrief opens on it.

Two quieter problems carry more than they look like they do. \textbf{Problem~8} --- how many of
the twenty sit inside the box --- is an early read on the misconception; a student who answers
``most of them'' will fail problem~14. \textbf{Problem~17} --- why the boxplot cannot show the
clump from 30 to 38 --- is the honest limit of the display, and it comes back as the AP Practice
page's MC~4.

\textbf{Two Science Sections is where the pen changes hands.} Problems 22--26, a fresh context,
worked entirely together. \textbf{Problem~26 is the AP-scored form} and the one that cannot be
...
do. If you only get through 22, 23, and~26, that is the right trade.
...
\textbf{Debrief (8 min), spoken.} Open on the \emph{almost}-right problem~14, then the count row,
then the comparison standard off the two problem~26 paragraphs you collected. If time is short,
...
minutes on the counting picture from notes problem~13 before starting 1.7. On the free response,
...
problem~26's paragraph in the Lesson~1.7 warm-up debrief rather than letting it ride into
