...
% PER NOTES ROW (the five numbers · anatomy · comparing distributions · one quarter, the crux —
% the crux row is LAST, straight before Guided Practice) →
...
\begin{frame}[t, plain]
  ...
  \sectionlabel{notes row 1, The Five Numbers $\cdot$ problems 1--3}
  \vspace{2pt}
  \begin{columns}[t]
  ...
  \vspace{6pt}
  \begin{center}
    {\small\color{slate} Problem~2: which \emph{part} of the plot marks $Q_1$? The box has two
    edges --- say which one.}
  \end{center}
\end{frame}
...
% ── NOTES ROW 2 ──────────────────────────────────────────────────────────────
\begin{frame}[t, plain]
  \navyheader{Anatomy: box, whiskers, and where a whisker has to stop}
  \sectionlabel{notes row 2, Anatomy of a Boxplot $\cdot$ problems 4--6}
  \vspace{2pt}
  \begin{columns}[t]
    \begin{column}{0.52\textwidth}
      \begin{center}
      \begin{tikzpicture}[scale=0.9]
        \bxp{0.9}{0.42}{0}{0.9}{1.9}{4.1}{8.0}
        \draw[->, thick] (-0.2,0) -- (8.6,0);
        \node[font=\tiny, above] at (0.9,1.35) {34.5};
        \node[font=\tiny, above] at (4.1,1.35) {50.5};
        \node[font=\tiny, below] at (0,-0.1) {30};
        \node[font=\tiny, below] at (8.0,-0.1) {74};
      \end{tikzpicture}
      \end{center}
    \end{column}
    \begin{column}{0.44\textwidth}
      \begin{itemize}\setlength{\itemsep}{6pt}
        ...
        \item A \textbf{whisker} reaches the most extreme value that is \redink{not} an outlier
              (EK UNC-1.L.2).
        \item Free throws: IQR $= 50.5 - 34.5 = \mathbf{16}$, fences $34.5 - 1.5(16) =
              \mathbf{10.5}$ and $50.5 + 1.5(16) = \mathbf{74.5}$. Nothing is outside them, so
              there is \textbf{no outlier} and both whiskers run to the extremes.
      \end{itemize}
    \end{column}
  \end{columns}
  \vspace{6pt}
  \begin{center}
    {\small\color{slate} An outlier \emph{would} get its own dot --- that is a \textbf{modified
    boxplot}, the version the AP exam draws. Problem~5: how many of the twenty sit \emph{inside}
    the box?}
  \end{center}
\end{frame}
...
\begin{frame}[t, plain]
  ...
  \sectionlabel{notes row 3, Parallel Boxplots $\cdot$ problems 7--9}
  \vspace{4pt}
  \begin{columns}[t]
...
\begin{frame}[t, plain]
  ...
  \sectionlabel{notes row 4, One Quarter $\cdot$ problems 10--14 --- the whole point of today}
  \vspace{6pt}
  \begin{itemize}\setlength{\itemsep}{8pt}
    \item Twenty players. Four sections. Your problem~10 counts: \textbf{5 \quad 5 \quad 5 \quad
          5}.
  ...
  \begin{block}{Essential Knowledge (UNC-1.L.2)}
    ...
    those values are --- \textbf{spread}, never \textbf{count}. And problem~14 is the price: a
...
\begin{frame}[t, plain]
  ...
  \sectionlabel[goldacc]{We do $\cdot$ problems 15--20 $\cdot$ you hold the pen}
  Two sections of the same course, one test, two five-number summaries side by side.
  \begin{itemize}\setlength{\itemsep}{5pt}
    ...
    \item \textbf{Problem 20:} write the comparison as \textbf{one paragraph}, in context.
  \end{itemize}
