%! Boxplots and Comparing Distributions — Unit 1, Lesson 1.6 — AP Practice (Answer Key)
% GRADUAL-RELEASE lesson: AP-format practice, assigned but NOT scored — the cover's score
...
% is two science sections, and this page runs on weekday-versus-weekend tips — a data set that
% appears nowhere else in the packet.
% The four MC items cover the four ways this topic is tested: 1 is the width-means-count
% distractor (the lesson's target misconception dressed as an AP answer choice), 2 is a
% five-number-summary/IQR reading item, 3 is mean-vs-median position under right skew
% (EK UNC-1.M.2), and 4 is the item whose correct answer is that the display CANNOT answer the
% question asked — the same limit students met in notes problem 17. Free-response (d) spirals to
% Lessons 1.3 and 1.4 (which display would show a second peak).
% Every student component in this course is 12pt.
% Page lockstep with ap_practice_key rests on \writelines{n} in the blank matching exactly n terse
% \ansline{} calls in the key, and on the \bxp macro and every TikZ block being byte-identical.
...
\pageheader{Unit 1, Lesson 1.6}{AP Practice --- Answer Key}
...
\begin{tikzpicture}[scale=0.85]
  ...
  \node[font=\scriptsize] at (4.2,-0.97) {Total tips for the shift (dollars)};
\end{tikzpicture}
\end{center}
\vspace{4pt}
...
\begin{enumerate}[leftmargin=1.9em, itemsep=0.13in]
  ...
  \item Which statement about the \textbf{weekend} shifts is best supported by the display?
        \begin{enumerate}[label=\textbf{(\Alph*)}, leftmargin=2.2em, itemsep=2pt]
          ...
          \item \ans{The same number of weekend shifts fall between \$88 and \$106 as fall between
                \$78 and \$88.}
  ...
  \item The weekend distribution is skewed to the right and has one unusually high value. Which
        ...
        \begin{enumerate}[label=\textbf{(\Alph*)}, leftmargin=2.2em, itemsep=2pt]
          ...
          \item \ans{The mean is greater than the median, because the long right tail and the high
  ...
  \item The manager asks whether weekend tip totals cluster around \emph{two} separate amounts
        ...
        \begin{enumerate}[label=\textbf{(\Alph*)}, leftmargin=2.2em, itemsep=2pt]
          ...
          \item \ans{This display cannot answer the question --- it reports only five summary
...
\begin{enumerate}[leftmargin=1.9em, itemsep=0.16in, resume]
  ...
  \item Use the tip records above.
        ...
        \ansline{Min \$42, $Q_1$ \$55, median \$62, $Q_3$ \$70, max \$84. The two whisker ends}\\[1pt]
        \ansline{are \$42 and \$84, the box edges are \$55 and \$70, the inside line is \$62.}
        ...
        \ansline{IQR $= 106 - 78 = \$28$, so $1.5 \times \text{IQR} = \$42$.}\\[1pt]
        \ansline{Upper boundary $= Q_3 + 42 = 106 + 42 = \$148$.}\\[1pt]
        \ansline{Since $158 > 148$, the \$158 weekend shift is an outlier.}

        \vspace{6pt}
        \textbf{(c)} Compare the weekday and weekend tip totals. Address center, variability, and
        unusual features, in context.\\[4pt]
        \ansline{Weekend tips were typically higher than weekday tips --- a median of \$88}\\[1pt]
        \ansline{against \$62. Weekend tips were also far more variable: an IQR of \$28 against}\\[1pt]
        \ansline{\$15, and a much wider overall span. The weekend shifts are skewed right and}\\[1pt]
        \ansline{include one outlier at \$158; the weekday shifts have no outliers.}
        ...
        cannot.\\[4pt]
        \ansline{Ask for a dotplot or a histogram of the weekend totals. A boxplot reports only}\\[1pt]
        \ansline{five numbers, so a gap between two groups of nights, or a second peak, is}\\[1pt]
        \ansline{hidden inside the box. A dotplot would show every night individually.}
...
\ansline{Answers vary. Published sentences usually name only which group is ``higher'';}\\[1pt]
\ansline{a strong one also compares variability and says so in the units of the variable.}
\end{extensionbox}
