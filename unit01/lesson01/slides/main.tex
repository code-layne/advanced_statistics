...
% practice) → YOU DO (independent practice, then the trailhead register) → debrief → close &
% assign. There is NO group activity in this course — the notes carry the whole release.
...
\begin{frame}[t, plain]
  ...
  \begin{block}{How today works}
    ...
    \textbf{three you work alone} $\to$ \textbf{the trailhead register, also on your own}
    $\to$ debrief.
  \end{block}
\end{frame}
...
% ── YOU DO, PASS 2: the transfer set (still individual — no group activity) ──
{
\setbeamercolor{background canvas}{bg=navy}
\begin{frame}[plain]
  \vspace{1.8cm}\hspace{0.8cm}%
  \begin{minipage}{0.86\textwidth}
    {\color{goldacc}\bfseries\small PUTTING IT TOGETHER \ \textbullet\ \ 10 MINUTES}\\[0.7cm]
    {\color{white}\bfseries\Large The Trailhead Register}\\[0.9cm]
    {\color{skymid}\small On your own. \textbf{No letter goes down until you can say why.}}
  \end{minipage}
\end{frame}
}

\begin{frame}[t, plain]
  \navyheader{Putting It Together: The Trailhead Register}
  \sectionlabel{You do $\cdot$ on your own $\cdot$ 10 minutes}
  Every party that hikes at Ross Hollow signs in and signs out. The ranger keeps the sheet
  all season --- and last year he added four new columns.
  \begin{itemize}\setlength{\itemsep}{5pt}
    \item \textbf{(a)} Classify all five recorded columns --- \redink{four of them written
          entirely in digits}. Only one of those four is quantitative.
    \item \textbf{(b)--(d)} Name the two calls people get wrong, then the percent trap and
          grouping, on ground you have not seen before.
  \end{itemize}
  \begin{block}{Ground rule}
    Run the amount test in your head before you write a letter down --- I will ask you for
...
\begin{frame}[t, plain]
  ...
  \begin{block}{This one is graded}
    This is your \redink{scored} homework, on paper, due next class. Start with 1, 3, and 5
...
\begin{frame}[t, plain]
  ...
  \begin{itemize}\setlength{\itemsep}{5pt}
    \item Four multiple-choice items on the trailhead register you just worked.
