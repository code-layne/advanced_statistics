...
\boxguard[30]
\begin{scenariobox}[The Trailhead Register, Revisited]{navy}
\small
The same Ross Hollow register you worked with in class, with the ranger's four new columns
merged in:
...
\begin{extensionbox}
\small
Find any data table --- a sports box score, a nutrition label, a product comparison. Write down
its column headings and classify each one. Then find a column written entirely in digits that
is \emph{not} quantitative, and explain what its digits are doing instead.\\[4pt]
\ansline{Answers vary. A box score works well: points and rebounds are quantitative,}\\[1pt]
\ansline{while jersey number and position are categorical.}
