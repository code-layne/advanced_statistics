...
% one Main Ideas / Notes table: I do (each row's lines and first problem) -> we do (the rest of
% the grid, then the Guided Practice row). THERE IS NO INDEPENDENT PRACTICE SET — the homework is
...
Guided Notes \& Practice & 34 & Fill the vocabulary box and the notes table: problems 1--15 row by row, then \emph{The Season Pass} (16--19) together, pen in their hand & \textbf{I do} each row's definition, picture, and first problem, \textbf{we do} the rest of its grid (24) $\to$ \textbf{we do} Guided Practice (10) \\
Debrief & 8 & Pens down, papers up --- answer aloud, nothing to fill in & Take the digit columns of the Amounts row (problem 11) public and run the amount test on each aloud \\
...
\begin{minipage}[t]{0.48\linewidth}
    ...
    \begin{itemize}
        ...
        \item \textbf{2.} ``Which of these lists could you add up and get something meaningful?''
              ...
              you circled have in common'' becomes the Amount Test row of the notes.
...
\begin{minipage}[t]{0.48\linewidth}
    ...
    \begin{itemize}
        ...
        \item \textbf{Debrief item~2 aloud} and leave their phrasing up. You will point at it
              during the Amount Test row and again in the closing debrief.
...
The notes are one \emph{Main Ideas / Notes} table --- five rows, 19 short problems, one big
picture per row. \textbf{In every row you read the definition, mark up the picture, and work the
first problem of the grid; the class works the rest, pen in their hand.} The vocabulary box is
filled \emph{as you go}, never in advance.
\begin{multicols}{2}
\textbf{Variable (5 min).} Read the definition, then mark up the hikers' table: write
\emph{categorical} under \emph{Trail} and \emph{quantitative} under \emph{Time out}, and say out
loud that 215 really is 120 more than 95, while Summit Trail is not ``more'' than Ridge Loop.
Work problem~1 yourself (a count is an amount); problems 2--4 are theirs.

\textbf{Amount Test (6 min).} Read the test, then mark up the picture: both averages compute,
4 bottles is a real amount, 4179.5 is nobody's permit --- ask for the party who holds it. The
silence is the argument. Label both boxes, then give them a minute for \emph{In your own words}.
Work problem~5 yourself; problem~6 is theirs and must end with the word \emph{label}.

\textbf{One Way Only (5 min).} Read the definition, then mark up the ruler: drop 95, 140, and
215 into their bands, trace the arrow down to the labels, and point at the crossed arrow back.
Point at the warm-up sentence still on the board when you say \emph{grouping}. Work problem~7
yourself (the average is 150 minutes) so that problem~8 lands: \emph{Medium} cannot give 140
back. Problem~9 must name a loss, not just say ``it becomes categorical''; problem~10 is the
same idea with two labels instead of three.

\textbf{Amounts (8 min) --- the crux.} Read the definition, then release the five-column strip,
problem~11: label \emph{Permit \#} yourself, and \textbf{expect the room to split on
\emph{Parking spot}} --- two-digit spot numbers look like small counts. Take a hand count on it
before revealing. \textbf{Problem~12 is the ordered-labels trap} --- Hard really is more than
Easy, but there is no number of anything. \textbf{Problem~13 is the crux}: average two spot
numbers and ask whose spot that is. \textbf{Problem~14 is the percent trap}: counting produces
numbers and the variable is still categorical. \textbf{Problem~15 is the sentence to protect}
--- the amount test stated without the word \emph{digits}. Do not skip it.

\columnbreak
\textbf{We do (about 10 min): The Season Pass.} A fresh context, problems 16--19, worked
entirely together. Students hold the pen; you hold the questions: \emph{``Pass number --- run
the amount test on it out loud.''} \quad \emph{``Miles traveled and locker number are both
digits. Say the difference in one sentence.''} \textbf{Problem~18 is where grouping gets used
rather than recited}: insist on \emph{what can no longer be computed}, not just ``it becomes
categorical.''

\textbf{The pen must actually change hands.} Guided Practice is the only place today where you
can watch a student decide, so do not narrate it. Put problem~16 on them cold, take a hand count
on \emph{Locker number} before anyone speaks, and make the room defend both answers.

Circulate and demand the reason, not the letter:
\begin{itemize}[leftmargin=1.1em, nosep]
  \item \textbf{Problem~16, the crux transferred:} ``Average two pass numbers. Now hand me the
        holder whose pass that is.''
  \item \textbf{Problem~17:} ``\emph{Miles traveled} and \emph{Locker number} are both digits. Say
        the difference in one sentence.''
  \item \textbf{Problem~18:} ``Name one thing the resort can no longer compute.''
\end{itemize}
While circulating, pick the papers to put up in the debrief --- one that got \emph{Locker number}
wrong in problem~16, one that justified it correctly in problem~17. \textbf{Your formative read
comes twice}: here, and again in the last eight minutes as students start the homework.
\end{multicols}
\end{skillbox}
...
\begin{enumerate}[leftmargin=1.3em, nosep, itemsep=3pt]
  \item \textbf{Put the digit columns from the Amounts row (problem~11) on the board and
        run the amount test on each aloud}, in this order: \emph{Water bottles} (easy, Q),
        \emph{Permit \#} (easy, C), \emph{Home ZIP} (C, and say ``80211 is not more
        than 80304''), \emph{Parking spot} (the hard one, problem~13 --- spot 17 is not more
        parking than spot 04).
  \item Put up the two Guided Practice papers you picked (problems 16--17) and name what the
        correct one did: \textbf{categorical variables are summarized by counting, not
        averaging.}
  \item Settle the percent trap in one line, off problem~14: the summary is numeric, the
        variable is not.
  \item Take grouping last, off problem~8. ``Medium'' cannot be turned back into 140 minutes.
\end{enumerate}
\columnbreak
...
\begin{itemize}[leftmargin=1.2em, nosep]
  \item \textbf{``It has digits, so it is quantitative''} (notes 11 and~13, GP~16, HW~1 and~4).
  ...
  \item \textbf{Ordered labels read as amounts} (notes~12, HW~1, \emph{Drink size} S/M/L). Large
        really is more than Small --- but there is no number of anything. Probe: \emph{``How
        much more?''}
  \item \textbf{Percent taken as proof of quantitative} (notes~14, HW~2, AP MC~3). Probe:
        \emph{``What are the values in that column --- Yes and No, or 62?''}
  \item \textbf{Grouping described but not costed} (notes~9, GP~18, HW~3). ``It becomes
        categorical'' is half an answer. Probe: \emph{``Name one thing you can no longer
        compute.''}
  \item \textbf{Counted quantities called categorical} (notes~1 and~11, \emph{Group size};
        \emph{Number of shots}). A count is an amount. Probe: \emph{``Is six more than two?''}
\end{itemize}
...
you whether the Amounts row landed. Sort what you see into three piles as you walk --- correct with a
...
the Amount Test row of the notes, with the permit-number picture (problems 5--6), where the
evidence does the work instead of your authority. Item~1 is a 60-second spiral to Lesson 1.0; item~3 takes 30 seconds and needs no
...
a definition to read, a picture to mark up, and a grid of short problems. \textbf{In every row
you read the definition, mark up the picture, and work the first problem; the class works the
rest with the pen in their hand.} The Variable row is quick --- one definition, the hikers'
table with its two boxes, four one-letter answers. \textbf{Spend the time in the Amount Test
and Amounts rows}, which carry the misconception. Mark up the averages picture live: both
averages compute, then ask for the party who holds permit 4179.5. The silence is the
argument.

The five-column strip in the Amounts row (problem~11) is where you find out who is with
you. \emph{Parking spot} is the one that splits a room, because two-digit spot numbers look
like small counts. Take a hand count before revealing, then let problem~13 --- the crux ---
settle it by averaging two spot numbers. Problem~12 is the ordered-labels trap, problem~14 is
the percent trap in a grid cell, and problem~15 --- the test stated without the word
\emph{digits} --- is the sentence the debrief will demand aloud, so make sure it is on every
paper.

\textbf{The Season Pass is where the pen changes hands.} \emph{Locker number} and \emph{Miles
traveled} sit side by side on the pass in problem~16 --- both digits, opposite answers --- and
problem~17 makes them say the difference. Problem~18 must end with a named loss (``the mean
age''), not just a classification. If you only get through 16, 17, and~18, that is the right
trade.
...
Lesson~1.2 by reteaching the Amounts row}, and say so plainly.
...
\textbf{Debrief (8 min), spoken.} The digit columns from problem~11, in the order given
...
the test \emph{without the word ``digits''} (their problem~15) is the one move that cannot be
