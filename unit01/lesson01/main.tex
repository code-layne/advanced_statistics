...
\begin{tcolorbox}[colback=sky, colframe=navy, boxrule=0.9pt, arc=2mm,
    ...
    Independent Practice and then \emph{The Trailhead Register} are where students carry it
    alone. The whole-class debrief at the end of the notes closes the loop.
\end{tcolorbox}
...
Guided Notes \& Practice & 38 & Fill the vocabulary box and notes 1--4; the Season Pass together; then Independent Practice and \emph{The Trailhead Register} alone & \textbf{I do} notes 1--4 (14) $\to$ \textbf{we do} the Guided Practice box (9) $\to$ \textbf{you do} Independent Practice and the register (15) while you circulate \\
Debrief & 8 & Fill the Debrief box at the end of the notes together & Take the four digit-columns public and run the amount test on each aloud \\
...
release, and the register at the end is the transfer, worked individually.
\begin{multicols}{2}
\textbf{I do (about 14 min).} Present notes sections 1--4 in order; students fill the blanks as
...
\textbf{We do (about 9 min).} The Guided Practice box, \emph{The Season Pass}. Students hold the
...
\columnbreak
\textbf{You do (about 15 min), in two passes.} First \emph{Independent Practice}, three items,
silent and alone, about 5 minutes. \textbf{Item~1 is the formative check} --- the coffee-shop
strip has one of each trap in it (\emph{Drink size} is a label that looks ordered;
\emph{Loyalty card \#} is digits that label). Sort what you see into three piles: correct with a
reason (ready); correct with no reason (ready, needs the language); marked Q (the misconception
survived). \textbf{If the third pile is more than a third of the room, stop and reteach
section~3} before releasing the register --- it will not work otherwise.

Then \emph{Putting It Together --- The Trailhead Register}, about 10 minutes, still individual.
This is the transfer: a data set they have not seen, four columns written entirely in digits, of
which only \emph{Water bottles} is quantitative. Item (b) asks them to name the two calls people
get wrong; (c) and (d) rerun the percent trap and grouping on fresh ground. Circulate and demand
the reason, not the letter:
\begin{itemize}[leftmargin=1.1em, nosep]
  \item \textbf{(a), the crux:} ``Average the two permit numbers. Now hand me the party whose
        permit that is.''
  \item \textbf{(a):} ``Is spot 17 more parking than spot 04?''
  \item \textbf{(c):} ``What are the values in the \emph{Dog along} column? Are they Yes and No,
        or are they 62?''
\end{itemize}
While circulating, pick the papers to put up in the debrief --- one that got \emph{Parking spot}
wrong, one that justified it correctly.
\end{multicols}
\end{skillbox}

\boxguard
\begin{skillbox}[Debrief --- whole class (8 min)]{sky}
Pens down, papers up, then fill the Debrief box at the end of the guided notes together.
\begin{multicols}{2}
\begin{enumerate}[leftmargin=1.3em, nosep, itemsep=3pt]
  ...
  \item Name what the correct papers did: \textbf{categorical variables are summarized by
        counting, not averaging.}
  \item Take register item~(c) and settle the percent trap in one line: the summary is numeric,
        the variable is not.
  \item Take item~(d) last. ``Long'' cannot be turned back into 215 minutes.
\end{enumerate}
\columnbreak
...
up. Item~3 is grouping in one line. This box is the only part of the notes filled in as a whole
class, so hold the room to it rather than letting students race ahead to the homework.
...
\begin{itemize}[leftmargin=1.2em, nosep]
  \item \textbf{``It has digits, so it is quantitative''} (notes~3, register~(a), HW~1 and~4).
  ...
  \item \textbf{Percent taken as proof of quantitative} (notes~3, register~(c), AP MC~3). Probe:
        \emph{``What are the values in that column --- Yes and No, or 62?''}
  \item \textbf{Grouping described but not costed} (notes~4, register~(d), HW~3). ``It becomes
...
\textbf{This one document is the lesson --- pace it 14 / 9 / 15, then 8 for the debrief.}
...
more than a third of the room, \textbf{stop and reteach section~3 before the register} --- it
will not work otherwise, and it is better to cut register items (c)--(d) than to run them cold.

\textbf{Putting It Together is the transfer, and it is individual work.} There is no group
activity in this course, so do not stage this as one: students have not seen this register, and
the point is that each of them can run the amount test without a table-mate to lean on. The
ground rule --- \emph{no letter goes down until you can say why} --- is worth enforcing for the
first two minutes even if it slows them down. The reasons are what the debrief harvests. Students
who finish early get the extension question: \emph{``find me a sixth column that would behave the
same way as Parking spot.''}
