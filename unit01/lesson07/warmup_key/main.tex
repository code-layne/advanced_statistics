...
% Mirrors ../warmup/main.tex exactly apart from the package swap, the \pageheader's second
% argument, and \blank -> \ans. Every \vspace, \arraystretch, and itemsep is identical, and each
% \ans is short enough to fit the blank it replaces, so the two files set on one page each.
...
\begin{enumerate}[leftmargin=1.7em, itemsep=0.13in]
  ...
  \item A set of quiz scores has a mean of \textbf{20} points and a standard deviation of
        ...
        3-point steps is that? \ans{6 points; 2 steps}
        \vspace{3pt}
