...
\begin{itemize}[leftmargin=1.1em, nosep, after=\vspace{2pt}]
  ...
  \item \textbf{Target misconception:} that the bigger raw number is the better result --- that a
        ...
        \textbf{Notes problem~17 is the trap} and \textbf{problem~19 its companion}: a negative
...
Guided Notes \& Practice & 34 & Fill the vocabulary box and the notes table: problems 1--20 row by row, then \emph{The Bread Loaf} (21--26) together, pen in their hand & \textbf{I do} each row's definition lines and first problem, \textbf{we do} the rest of its grid (24) $\to$ \textbf{we do} Guided Practice (10) \\
Debrief & 8 & Pens down, papers up --- answer aloud, nothing to fill in & Open on an \emph{almost}-right answer to problem~18, then run problems 17, 19, and~20 public \\
...
\begin{minipage}[t]{0.48\linewidth}
    ...
    \begin{itemize}
        ...
        \item \textbf{Debrief item~3 aloud} and leave \emph{``symmetric, so split the leftovers
              ...
              problems 23, 24, and the AP practice all reach back for it.
        \item The percentages in item~3 are 90 / 10 / 5 rather than 95 / 5 / 2.5 --- keep it
              that way, so that the empirical rule's real numbers arrive in notes problem~13
...
\begin{multicols}{2}
\textbf{Parameter \& Statistic (5 min).} All seven lines, then the four symbols on the board in
two columns --- $\mu$, $\sigma$ beside $\bar x$, $s_x$ --- and leave them up for the rest of the
period; this contrast pays off in every remaining unit. Work problem~1 yourself; 2--6 are theirs.
\textbf{Problem~6 is the one to check}: every athlete in the field was measured, so the symbol is
$\mu$.
...
Dev's, and say out loud that 12.0 metres and 12.3 seconds are not on one scale. Work problem~7
yourself ($+3.0$ m); 8--12 are theirs. \textbf{Problem~11 is where the sign becomes meaning}:
insist on the direction \emph{and} on ``good in a sprint.'' Problem~12 is $z = 0$.

\textbf{Normal distribution \& Empirical rule (10 min).} Fill the shape lines and the three
percentages, pointing at the given curve. Then \textbf{problem~13 is the arithmetic that earns
the rule}: they add the band strip and fill both records --- 341/476/499 out of 500, and
273/381/399 out of 400. \textbf{Do not announce 68--95--99.7 before that table is filled.} Their
own numbers are 68.2, 95.2, 99.8 and 68.25, 95.25, 99.75; problem~15 rounds them into the rule.
Problem~14 is the point: shape, not units, is what fixes the percentages.

\columnbreak
\textbf{Whose day was better? (5 min) --- the crux.} Read the coach's argument aloud exactly as
written, then fill the boxed sentence. \textbf{Problem~17 is the trap} --- take a hand count on
``is the coach right?'' before anyone speaks. Work nothing here yourself; this is the row where
you find out who is with you. \textbf{Problem~18 settles it with the two $z$-scores} and must end
with a named athlete and a reason. \textbf{Problem~19 is the companion error} (negative read as
worse) and \textbf{problem~20 is the counterweight} --- a skewed or two-peaked set does not obey
the rule.

\textbf{We do (about 10 min): The Bread Loaf.} Problems 21--26, worked entirely together, on the
same bakery the notes already pictured --- so no new context has to be loaded in ten minutes.
...
\textbf{Problem~25 is the judgment problem} and \textbf{26 is the honest limit} --- protect time
for both; if you only get 21, 23, 25, and~26, that is the right trade.

Circulate and demand the reason, not the number:
\begin{itemize}[leftmargin=1.1em, nosep]
  \item \textbf{Problem~17, the crux:} ``Write the unit after each number and read your sentence
        back to me.''
  \item \textbf{Problem~19:} ``Below the mean \emph{time}. Faster or slower than the field?''
  \item \textbf{Problem~24:} ``Five percent is outside on \emph{both} ends. How much on the end
        you were asked about?''
\end{itemize}
While circulating, pick the papers for the debrief --- one \emph{almost}-right problem~18 (both
$z$-scores computed, direction dropped) and one problem~20. \textbf{Your formative read comes
...
\begin{enumerate}[leftmargin=1.3em, nosep, itemsep=3pt]
  \item \textbf{Open on the \emph{almost}-right problem~18} you collected: both $z$-scores
  ...
  \item Then problem~17, the trap, in public: metres and seconds, two fields, never one scale.
        Write $+3.0$ m and $-0.9$ s beside $2.0$ and $-1.5$ and make someone say why only the
        second pair can be compared.
  \item Problem~19 next --- the sign. \emph{``Would you rather be above or below average in a
        100-metre time?''} Take a hand count, then settle it.
  \item Problem~20 last, off their own problem~13 numbers: the rule is a fact \emph{about a
...
\begin{itemize}[leftmargin=1.2em, nosep]
  \item \textbf{``Dev, because 12.3 is bigger''} (notes~17, HW~3 --- the target misconception).
...
it there all period --- notes problems 23 and~24, the homework, and the AP practice all reach
back for it. Item~3's percentages are 90 / 10 / 5 rather than the real 95 / 5 / 2.5 on purpose:
the real numbers arrive in notes problem~13 out of the students' own band counts, and they land
...
short problems. \textbf{In every row you fill the definition lines and work the first problem;
the class works the rest with the pen in their hand.} Row~1 is quick --- seven lines, six
one-letter answers --- and the only thing worth slowing for is putting $\mu$, $\sigma$, $\bar x$,
$s_x$ on the board in two columns and leaving them there.

\textbf{Row~2 is where the arithmetic becomes an idea.} The dotplots are given: point at the two
gold dots and say out loud that 12.0 metres and 12.3 seconds are not on one scale. Work
problem~7 yourself so the subtraction is settled, then release 8--12. \textbf{Problem~11 is the
one to read while circulating} --- a student who writes ``1.5 below'' without saying that below
is \emph{good} in a sprint has half the idea.

\textbf{Row~3's problem~13 is the table that earns the empirical rule --- do not announce
68--95--99.7 before it is filled.} The two records are deliberately unrelated: 341/476/499 of 500
loaves and 273/381/399 of 400 temperature readings, which come out 68.2, 95.2, 99.8 and 68.25,
95.25, 99.75. Write their six numbers on the board \emph{first}, then round them into the rule in
problem~15. Problem~14 is the sentence that matters: shape, not units and not sample size, is
what fixes the percentages.

\textbf{Row~4 is the crux, and you work none of it.} Read the coach's argument aloud exactly as
written and take a hand count on problem~17 before anyone speaks; the split in the room \emph{is}
the lesson. Problem~18 must end with a named athlete and a reason that mentions direction.
Problem~19 is the companion error --- negative read as worse --- and it is the one that survives
into the homework's item~5. Problem~20 is the counterweight and the AP-tested half of the rule.

\textbf{The Bread Loaf is where the pen changes hands.} Problems 21--26 on the bakery the notes
already pictured, worked entirely together. Problem~23 is the halve-the-tail move from warm-up~3
in its real numbers; \textbf{problem~25 is the judgment problem and 26 the honest limit}, and
they are what to protect if the clock is tight --- 21, 23, 25, 26 is the right subset.
...
\textbf{Debrief (8 min), spoken.} Open on the \emph{almost}-right problem~18 --- both $z$-scores
computed, the direction dropped --- because that is the exact gap the exam punishes. If time is
short, cut problem~20 to one sentence; making a student state a full $z$-score interpretation in
...
beside the skewed one and ask what is missing --- that is notes problem~20 again, in AP wording.
...
idea. Item~5 is notes problem~19 in AP clothing --- the direction reversed, in a race where a
