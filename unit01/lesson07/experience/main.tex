...
%
% Every \begin{work} block is authored BYTE-IDENTICALLY here and in experience_key/ — under
...
\pageheader{Unit 1, Lesson 1.7}{Experience \& Formalize: Two Tryouts}
...
\begin{enumerate}[label=\textbf{\alph*.}, leftmargin=1.8em, itemsep=4pt]
  \item Maya's result is 12.0, Dev's is 12.3. Whose \emph{number} is bigger? \blank{2.6cm}
  ...
  \item How far is each result from that athlete's \emph{own} event average?
        ...
        \end{work}
        Does \emph{that} settle it? \blank{2.0cm} Say why it still does not.
        \answerspace{1.4cm}{}
  ...
  \item Jordan cleared exactly the average height for the high-jump field. What does part (c)'s
        method give for Jordan? \blank{2.0cm} What would a \emph{negative} result mean?
        \answerspace{1.2cm}{}
\end{enumerate}
\end{scenariobox}
...
\hline
1 standard deviation & \blank{1.7cm} & \blank{1.7cm} & \blank{1.7cm} & \blank{1.7cm} \\
2 standard deviations & \blank{1.7cm} & \blank{1.7cm} & \blank{1.7cm} & \blank{1.7cm} \\
3 standard deviations & \blank{1.7cm} & \blank{1.7cm} & \blank{1.7cm} & \blank{1.7cm} \\
\end{tabularx}
...
\begin{itemize}[leftmargin=1.1em, itemsep=2pt]
  \item \textbf{Parameter:} a numerical summary of a \blank{2.4cm}. \textbf{Statistic:} a
        numerical summary of a \blank{2.0cm}. The parameters \blank{1.2cm} and \blank{1.2cm}
        describe a population; $\bar x$ and $s_x$ (Lesson 1.5) describe a sample.
  \item \textbf{Normal distribution:} a curve that is \blank{2.8cm} and \blank{2.4cm}, fixed
        entirely by $\mu$ and $\sigma$. Real data is never exact, so we say \blank{3.0cm} normal.
  \item \textbf{Empirical rule.} Approximately normal $\Rightarrow$ about \blank{1.7cm} of
        observations within 1 standard deviation of the mean, \blank{1.7cm} within 2,
        \blank{1.7cm} within 3.
  \item \textbf{Standardized score ($z$-score).} $z = (x_i - \mu)/\sigma$ counts how many
        \blank{4.0cm} the value sits from the mean. In context: ``a 830 g loaf has $z = 1.5$, so
        it is \blank{4.9cm} above the mean weight of 800 g.''
  \item $z < 0$: the value is \blank{2.0cm} the mean. $z = 0$: \blank{2.6cm} it. Carrying no
        units, $z$-scores and \blank{2.4cm} report \emph{relative position}, so they compare two
        different data sets.
  \item Technology (calculator or normal table) turns a $z$-score into an exact \blank{2.4cm}.
        That is Unit~5.
\end{itemize}
\end{tcolorbox}
